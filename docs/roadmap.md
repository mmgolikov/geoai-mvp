...
4. Request-scoped Auth kernel: a single email-or-phone entry screen, email sign-in link, phone SMS OTP, exact-target PKCE/session/logout, permanent-user verification and verified-identity Admin/Onboarding routes are implemented locally and statically verified. MFA is deliberately out of scope. The mock `demo@geoai.space` / `111111` session is browser-only and cannot authorize protected APIs. AUTH-01B Product repositories and every runtime persona-readiness flag stay disconnected/false. Next configure an SMS provider if phone delivery is required, create an owner-approved confirmed user, bootstrap v2, and prove the full email/phone HTTP/JWT/RLS/IDOR/Admin matrix on a clean Preview.
