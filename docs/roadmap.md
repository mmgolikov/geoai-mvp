...
4. Request-scoped Auth kernel: SSR cookie, claims/user/profile verification and exact action/capability contracts are staged; bind verified organization/project membership and resource RPCs to routes, then prove the HTTP/JWT/IDOR matrix. Never authorize users through service role.
...
6. Source custody/visibility: the pending five-table RLS-closed model, immutable release/artifact/status/receipt records, actor/tenant FKs, fail-closed `restricted`/`registered_unverified` backfill, approved-only bounded no-path `api.current_source_releases()` and static checker are staged. Next execute replay and real source-read personas, design the trusted worker/idempotent receipt write path, close rights/attribution/retention/rollback, and keep providers blocked until explicit activation. Zero/manual/planned sources cannot become report evidence.
