...
2. Canonical database migration: exact live-ledger reconstruction, local `api`-only Postgres 17 config, pinned Supabase CLI `2.109.1`, three-migration operator guard and static containment/identity/source-custody contracts are staged. Documentation/evidence snapshot `f39dcee18c2601b88ae51e061627925037d1aa77`, over functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`, passed Quality Gate run `29497314994`; DB job `87617248275` completed clean start/reset and all 71 assertions. The 14 red-team additions cover exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and artifact/status-event/ingestion-receipt update/delete immutability; the policy sweep covers every custody/audit table for `public` or authenticated-applicable policies. Next prove upgrade replay, zero drift, direct-`public` denial, positive/negative HTTP/JWT/source personas, advisor parity and owner-controlled live apply.
...
3. Retain the exact-snapshot application/database Quality Gate and artifact, then add upgrade replay, HTTP Auth/RLS/source persona E2E, IDOR and adversarial upload tests to CI.
...
6. Remove the disabled legacy AnalysisPanel fanout/dead effects, aggregate or bound the six-request Hub bootstrap, retain compact 48/64 KB source and output-trace budgets, keep the public report-package collection summary-only/16 KB and scope `preserveDrawingBuffer` to capture. Exact-docs-head Preview `dpl_Gh5btUKs8yzySxvpxGnwXjMkYayK` proves compact remote route bodies (4,292–18,284 B across the six recorded source/readiness/lineage routes), superseding the old 133–158 KB baseline; browser-flow, bundle and Core Web Vitals evidence remains open because HTTP checks are not browser evidence and the local headless runner was blocked by a system Unix-socket restriction.
