...
2. Canonical database migration: head `e999c5a07d3ced6c95f2eb44f6a5f03a9c17caea` passed clean 71/71, a synthetic local exact-ten-ledger-prefix/pre-ledger-repair/three-pending-migration/security-surface rehearsal and a second 71/71. This is not a current-development clone, live-derived upgrade replay, drift, live apply or DB-01 certification. Next prove those live-derived gates, direct-`public` denial, HTTP/JWT/source personas and advisor parity.
...
4. Request-scoped Auth kernel: permanent-user evidence now requires matching UUID claims/user subjects and explicit non-anonymous markers before profile RPC; mismatch is 401 and anonymous identity 403. AUTH-01B stays disconnected, all readiness false. Next execute DB/ENV gates, bind reviewed reads and prove HTTP/JWT/RLS/IDOR personas.
