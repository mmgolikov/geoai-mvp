...
1. Data API and identity model decision: follow the [draft containment runbook](SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md) to choose disable-Data-API or dedicated-minimal-`api`-schema; choose and align global profile, organization membership and project membership semantics. No live apply is authorized.
2. Canonical database migration: reconcile historical schemas/anonymous grants, profile/Auth uniqueness/FK, membership nullability/uniqueness/consistency and AOI write roles; prove clean/upgrade replay, zero drift and positive/negative RLS personas.
...
4. Request-scoped Auth kernel: validate the caller JWT/user, profile, organization, project membership and role on the server through a caller-scoped Supabase client; never authorize users through service role. Ephemeral development may run in parallel, but integration cannot close before the schema decision.
5. Protected Storage: derive scope server-side, validate full body and file magic, checksum/quarantine/AV state, and verify owner/project-bound `storage.objects` policies and signed URLs in caller context.
6. Source custody/visibility: explicit public-demo/project-private/operator visibility, rights/attribution, checksums, retention and a compact public DTO without internal paths/live probes/deep anonymous snapshot traces; zero/manual/planned sources cannot become report evidence.
