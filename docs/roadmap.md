...
2. Canonical database migration: all six candidates replayed on rehearsal; hosted pgTAP is `183/183`; all 29 GeoAI domain tables have RLS and uncovered domain FKs are zero. Table-level two-session lock-order runtime evidence now passes without deadlock or residual rows. Next prove the same concurrency through authenticated invitation RPCs, real HTTP Auth/MFA, resource-specific Admin pagination, Storage personas and the separately authorized development upgrade/drift/apply plan. [Receipt](SUPABASE_AUTH_REHEARSAL_RECEIPT_2026_07_16.json).
...
4. Request-scoped Auth kernel: exact-target PKCE/session/logout, permanent-user verification, TOTP MFA and AAL2 Admin/Onboarding routes are implemented locally and statically verified. AUTH-01B Product repositories and every runtime persona-readiness flag stay disconnected/false. Next close ENV-01, create an owner-approved confirmed user, bootstrap v2, and prove the full HTTP/JWT/RLS/IDOR/MFA/Admin matrix on a clean Preview.
