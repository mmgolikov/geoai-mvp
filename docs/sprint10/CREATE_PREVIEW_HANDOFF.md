...
- Initialisation, import, MapLibre error and WebGL-unavailable states retain the original 2D plan as fallback. The saved result remains unchanged.
- Map teardown removes the MapLibre instance and listeners; the resize observer and deferred resize frame are also cancelled.
...
Result: PASS — exact saved A/B geometry, actual heights/bases, immutability, local tile-free MapLibre contract and fallback contract.
...
Result: 2/2 PASS in 7.3 seconds. The owned server was stopped after the run.
