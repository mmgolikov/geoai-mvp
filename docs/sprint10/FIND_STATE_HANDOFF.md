...
- Browser-local save and view-update commits use the shared identity operation queue, bounded stable-store verification and a final synchronous raw-store comparison. When same-page project state changes during an awaited hash, the mutation is rebuilt from the latest verified store; it updates only the initiating project/artifact and preserves the current active project plus all unrelated project bytes.
