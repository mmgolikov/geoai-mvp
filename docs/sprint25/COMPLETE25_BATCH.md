...
8. The outer `finally` retires both personas even when claiming, creation, authentication, acquisition or batch work throws. A full batch takes at least about four hours, longer than the original one-hour access tokens. Batch-only retirement therefore performs one bounded password reauthentication of each SAME exact user, verifies returned user/session IDs, email, fresh expiry and authenticated `getUser`, then uses this fresh token for global revoke and the strict post-ban stale-JWT profile check. The original two refresh tokens are preserved and must still be rejected; the additional independent retirement refresh token must also be rejected. A returned same-user session is retained for best-effort cleanup even when freshness/verification fails; a positively different user is never authorized for revocation. No new accounts, looped reauthentication or relaxed expired-token 401 acceptance is introduced. Retirement failure produces `FAIL_ACTION_REQUIRED`; it never becomes a batch PASS. Legacy single-child once-only behavior remains available and unchanged. Credentials stay in memory/whitelisted child environment; no Admin or publishable key is forwarded to a browser child or persisted.
...
`complete25-batch-retirement-check.mjs`: invokes the actual retirement implementation with expired-original JWT sentinels and fake SDK/fetch only. Fifteen cases cover success, denied reauth, wrong user/session/email, expired/short fresh token, duplicate refresh token, missing token, failed/mismatched `getUser`, logout 401, accepted/rate-limited additional refresh, and stale-profile 401. Every branch still attempts the expected user's ban/read-back and clears credentials; 401 is never turned into revocation or stale-profile proof. Batch receipts add separate safe counts for same-user retirement session verification and the additional refresh denial; the original four refresh-denial count remains unchanged.
