...
| GitHub Quality Gate | Run `29496255541`, partial/overall failure; do not cite as successful |
| Application job | `87613914544`, failure at stale pre-publication documentation current-truth; new all-green exact-head run required |
| Supabase clean replay/pgTAP | Job `87613914543`, success; clean start + deterministic reset; `Files=1, Tests=71`; `All tests successful`; `Result: PASS` |
| Evidence artifact | Not current: require a new artifact from the post-publication all-green Quality Gate |
| Vercel Preview | `dpl_EaXsfjW5y2WUkCoXoeTYmGiD1DiY`, READY on exact functional content SHA; [deployment](https://geoai-ltagjva86-geoaidev.vercel.app) |
