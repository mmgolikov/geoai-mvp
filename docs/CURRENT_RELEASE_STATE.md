...
The current working tree supersedes the earlier MFA-heavy candidate with one simple login surface. The landing header, hero and shared product navigation now expose an explicit sign-in/account action on desktop and mobile rather than leaving `/login` discoverable only by direct URL. Email sends a PKCE sign-in link and creates the user when needed; phone sends/verifies an SMS code once an external SMS provider is configured; `demo@geoai.space` / `111111` creates browser-only sample state that never authorizes protected APIs or Admin. `/register` and `/mfa` redirect to `/login`, the callback defaults to `/workspace`, and onboarding hides the invitation token while retaining fragment-to-HttpOnly-cookie staging and SHA-256-only database handoff. Application Admin requests require a verified permanent identity without MFA. Migration `20260716213214_simplify_auth_remove_mfa_requirement.sql` prepares the equivalent database override but has not been applied to rehearsal, development or Production. TypeScript, production build and the revised Auth/Admin/static access contracts pass locally; `/workspace` and `/explore` are 221 kB First Load JS in this build. This is implementation/static evidence, not hosted Auth activation: real email/phone confirmation/session/logout, banned/deleted/unconfirmed/anonymous denial, rendered-browser Admin/Onboarding, authenticated RPC concurrency, resource-specific Admin pagination, first real owner, SMS delivery, Storage, development apply, a fresh exact-head Preview, real sources and Production remain open.
...
| --- | --- |
| Candidate remote head | `8e0039260f4cf201b230288b6b02c48d2955600e`, Draft PR #97; `main` unchanged |
| Git tree | `4c4f007e74f189bf5acdb5de66f352cf9cd59edb` |
| GitHub Quality Gate | Run `29534323096`, success |
| Application job | `87741880884`, success |
| Supabase replay/rehearsal/pgTAP | Job `87741880992`, success; clean replay, synthetic ledger-prefix upgrade rehearsal and second pgTAP pass completed |
| Quality artifact | `8389906783`, `geoai-quality-evidence-29534323096`, digest `sha256:a59d7ea7323665abf51ca0dd75ec3cedc67768dcf532f7935c545417487a5f71` |
| Database artifact | `8389970353`, `geoai-database-evidence-29534323096`, digest `sha256:f8fc827cec263848def2f25d29da99900a3cc820b3b3cf56c25859d367817962` |
| Exact Auth-rehearsal Vercel Preview | `dpl_66rk4tVny9TmPjo7BKona5Xo1p1b`, READY on exact candidate head; [deployment](https://geoai-jmlv4t456-geoaidev.vercel.app) |
| Preview Auth/negative matrix | `/login`, `/register`, `/mfa`, `/onboarding`, `/admin`, `/api/auth/session`, `/api/db/health` and `/api/platform/activation-status` returned 200; Auth session reports `supabase_auth`, hard access, `allowDemoPublic:false`, no demo identity and anonymous `claims_unverified`; private/no-store and security headers are present; no Auth-route runtime error cluster was observed |
| Browser evidence | Rendered browser/mobile/keyboard/print and real email/phone/Admin personas remain unclaimed. The table above describes the pre-simplification hosted Preview; the current simplified flow still requires a fresh exact-head Preview and rendered checks. |
