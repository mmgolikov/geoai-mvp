...
Navigation: [Documentation Index](DOCUMENTATION_INDEX.md) · [Product Baseline](PRODUCT_BASELINE_AND_READINESS.md) · [External Authority Registry](EXTERNAL_AUTHORITY_REGISTRY.json) · [WP-DEV13-002 Phase A](WP_DEV13_002_EXACT_HEAD_EVIDENCE_CLOSURE_PHASE_A.md) · [Source Audit](PRODUCTION_BASELINE_SOURCE_AUDIT_2026_08_18.md) · [Architecture](architecture.md) · [Data Strategy](data-strategy.md) · [QA Checklist](qa-checklist.md)

## Current externally verified release baseline

Evidence as of `2026-08-18T18:03:54Z`:

| Item | Verified state |
| --- | --- |
| GitHub release | PR [#113](https://github.com/mmgolikov/geoai-mvp/pull/113), merged |
| `main` and `release/production` | `7f323c4227f2409f3fe2d4d68be48a30176f4e2a` |
| Vercel Production | `dpl_4yBHCo1eZ7N6GYQWGAg1EdQGwFTE`, READY, target Production, exact Git SHA |
| Production alias | https://geoai-mvp.vercel.app |
| Product stage | `public_demo_prototype` |
| Runtime mode | `public_demo_only`, `browser_local`, `demo_public`, soft access |
| Protected operation | Confidential pilot, real Auth/RBAC, protected Storage and real-source custody blocked |
| S0 blockers | `DB-01`, `AUTH-01`, `STORAGE-01`, `SOURCE-01` |
| Historical green CI | Quality Gate `30158549978`, success on the exact merge SHA |
| Latest CI | Quality Gate `31946738874`, partial/failure: DB replay passed; application stopped at dependency audit before later checks |
| Rollback | Not re-certified in WP-DEV13-001; no Production change was made |
| Maturity | Not Production-ready and not pilot-ready |

The exact release chain is confirmed from GitHub refs and PR merge through the Vercel deployment/alias to the live health response. Repository evidence is time-boxed: this release/runtime claim expires at `2026-08-25T18:03:54Z`, and live GitHub/Vercel state supersedes it sooner. The documentation gate fails after expiry. See the machine-readable [External Authority Registry](EXTERNAL_AUTHORITY_REGISTRY.json) and the permanent [Source Audit](PRODUCTION_BASELINE_SOURCE_AUDIT_2026_08_18.md).

The released Production runtime, including the PR #113 Product System v3.2.2 correction, is the current visual/product baseline. Historical v3.2.1 design receipts remain provenance; live Figma parity is only partial. Draft PR #143 is `excluded_non_authority` and must not be copied, cherry-picked, recreated or salvaged without a separate approved Change Request.

## Active product and readiness boundary
...
The following point-in-time tuple is retained as historical evidence only and is superseded by the PR #113 baseline above and by newer external evidence.
...
The historical JSON snapshot remains at [LAST_VERIFIED_RELEASE_SNAPSHOT.json](LAST_VERIFIED_RELEASE_SNAPSHOT.json) and is registered as `superseded_do_not_use_as_current` in the [Superseded Register](SUPERSEDED_DOCUMENT_REGISTER.md).
...
The historical PR #97 full-system audit remains valid as containment evidence, while the PR #106 tuple is the previously verified repository snapshot superseded by the current PR #113 external release chain.
...
## Historical design authority — read-only verification
...
Founder approval in GitHub issue #110 and the v3.2.1 receipts remain historical design provenance. Their old branch fields such as `mergeAuthorized:false` and `productionAuthorized:false` describe the pre-release candidate, not current runtime status. PR #113 subsequently released the Product System v3.2.2 correction. No live Figma read in WP-DEV13-001 proves exact current parity, so Production is the visual baseline and Figma status remains `partial`. No Figma/Code Connect write or new body redesign is authorized.
