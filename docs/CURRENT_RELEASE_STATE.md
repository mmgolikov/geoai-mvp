...
Founder approval in GitHub issue #110 supersedes that earlier identity restriction for the bounded CR 10.02 candidate only. The isolated `design/cr-10-02-foundation-shared-shell` branch may implement Product System v3.2 semantic tokens, identity node `468:57`, one responsive shared Product shell and the four approved primitives from exact baseline `d788ea4ddeecc719b5ffcecdd6aab8539cc9b755`. Page bodies, Page 90/Page 99, Figma/Code Connect writes, merge and Production remain excluded. This candidate is not part of the released tuple above; exact-head Draft PR/CI/Preview evidence is recorded in [the CR](CR_10_02_DESIGN_FOUNDATION_SHARED_SHELL.md) and [QA receipt](DESIGN_FOUNDATION_SHARED_SHELL_QA_RECEIPT.md).
