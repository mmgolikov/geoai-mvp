...
Final hosted evidence is `71/71 + 73/73 + 39/39 = 183/183` pgTAP personas. The final 39 prove invitation expiry persistence/replay denial, structural organization→project→invitation lock order, bootstrap-v2 provenance, temporary-ban/profile behavior and fail-closed initial-only Admin snapshot pagination. Test transactions leave no additional Auth users, but the active hosted rehearsal currently has one pre-existing confirmed Auth user. That user was not created by CR 09.22, has no project membership authority, no tenant authority and no protected-resource authority, and no identifying user data is recorded in repository evidence. All `29` GeoAI domain tables excluding managed `spatial_ref_sys` have RLS; uncovered domain foreign keys are `0`. PostgREST is still pinned to `pgrst.db_schemas=api`, with `14` allowlisted functions and no API relations. Existing HTTP evidence proves `api.healthcheck()` 200/healthy, `public` 406/`PGRST106`, and an API base-table probe 404/`PGRST205`; remediation did not change API signatures or schema exposure.

Two independent hosted PostgreSQL sessions also completed canonical organization→project→invitation transitions for concurrent `create→accept` and `create→revoke` paths with a five-second lock timeout, no deadlock and zero fixture rows after rollback. This closes the table-level lock-order runtime regression only. It does not prove the authenticated `api.create_invitation()`, `api.accept_invitation()` and `api.revoke_invitation()` HTTP paths because CR 09.22 executed no real-user browser persona and the one pre-existing rehearsal Auth user has no membership, tenant or protected-resource authority.
...
Founder approval in GitHub issue #110 supersedes that earlier identity restriction for the bounded CR 10.02 candidate only. The isolated `design/cr-10-02-foundation-shared-shell` branch may implement the Product System v3.2.1 accessibility correction over the unchanged semantic baseline, identity node `468:57`, one responsive shared Product shell and the four approved primitives from exact baseline `d788ea4ddeecc719b5ffcecdd6aab8539cc9b755`. Accessibility receipt `1819:11` separates enabled inactive `#606f83` from disabled `#667587`; no Axe/WCAG exception is authorized. Page bodies, Page 90/Page 99, Figma/Code Connect writes, merge and Production remain excluded. This candidate is not part of the released tuple above; exact-head Draft PR/CI/Preview evidence is recorded in [the CR](CR_10_02_DESIGN_FOUNDATION_SHARED_SHELL.md) and [QA receipt](DESIGN_FOUNDATION_SHARED_SHELL_QA_RECEIPT.md).
