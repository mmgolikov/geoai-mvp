...
- Bind direct attribute questions to the exact requested allowlisted field and its attribute receipt. Any roof, rooftop, facade, exterior or cladding question, including colour synonyms such as shade, is outside the current allowlist and must be `unsupported`; object identity or geometry alone cannot stand in for that field. For a supported height, level count, construction date, style, surface or accessibility field, the visible answer is rendered deterministically from the canonical key/value and receipt rather than from model prose.
...
- A direct answer about an absent attribute, including roof colour/shade or facade/exterior properties, fails closed; a supported attribute is reduced to its canonical source value, so one valid value cannot legitimise appended invented claims.
