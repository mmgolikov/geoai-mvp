...
- replace the fragmented data models with four versioned contracts: `GeoContextSnapshot`, `ScenarioRegistry`, `DecisionRecord` and the downstream `DecisionRenderReceipt`;
...
`Analyse / Find -> shortlist -> compare -> object dashboard -> Create alternatives -> evaluate -> finalized DecisionRecord -> report/dashboard/Project Summary artifact -> external DecisionRenderReceipt -> Project Hub index`
...
| GeoContext and evidence | V5.1 live evidence pack, deterministic point-to-object contract, source-lineage utilities | **Adapt** | Make locale-independent `GeoContextSnapshot V1` the canonical immutable truth envelope across Analyse, Find, Create, compare and report. Map V5 `derived` to V1 `calculated`; distinguish modelled output from screening hypothesis. Replace coarse rights state with a versioned operation/channel/delivery/territory rights-scope receipt. | Strict schema, semantic graph, exact hash, coverage/freshness/confidence/gap, rights-scope and tamper fixtures pass. No negative observation becomes absence without measured coverage; unknown rights fail closed. |
...
| Reports/export | `components/reports/*`, print primitives/routes and `report-package/*` | **Adapt** | Reuse rendering/print/accessibility primitives. Freeze only template IDs/versions in the parent DecisionRecord; finalize it first, then create the artifact and an external `DecisionRenderReceipt`. Report generation cannot call analysis/ranking again or write child hashes back into the parent. | Parent/artifact/receipt hashes form an acyclic chain; report values match the source DecisionRecord; caveat, source/time/coverage/gaps and validation requirements are present; deterministic same-input re-render test. |
| Project Hub / data room | Project dashboard, browser artifact index, repository adapters and project-key fail-closed behavior | **Adapt** | Use Project Hub as an index of DecisionRecords, snapshots, DecisionRenderReceipts, validation tasks and source artifacts. Preserve local/public-demo mode until protected persistence is explicitly activated. | Unknown/wrong project fails closed; cross-project references rejected; `project` and `export` gates are independently enforced; browser/local and hosted modes are visibly distinct. |
...
| Data types and persistence | `project-workspace-types.ts` (`unknown` payloads), browser stores, repository adapters, staged point-object persistence migration | **Replace contract; adapt adapters** | Introduce typed snapshot/registry/decision/render-receipt DTOs first. Add storage adapters only after immutable IDs/hashes, tenant/project scope and retention are defined. No hosted write is part of this package. | Compile-time DTO parity, strict JSON Schema validation, idempotency/tamper/acyclic-render tests, then separate migration/RLS approval and replay. |
| Integrations | V5.1 Photon/Nominatim/Overpass/OpenAI; source registry and connector foundations | **Adapt** | Put every provider behind a source/method adapter that emits receipts. Provider output is never directly rendered or sent to a model. Add sources only after a versioned rights-scope receipt explicitly covers operation, channel, delivery mode, territory, redistribution/derivative/share-alike terms, evidence, expiry/review, custody, coverage, freshness and rollback. | Allowlist, timeout, size/rate limit, response minimization, complete permission-partition and provenance tests; unknown/missing scope blocks affected model input, ranking and export; source failure produces partial/blocked context without fabrication. |
...
| QA and release governance | Existing V5.1 source checks, E2E, build, data-honesty, exact-head Preview receipts | **Adapt** | Add strict Ajv2020 schema compilation, semantic cross-reference, shared operation vocabulary, canonical hash, rights fail-closed, same-cohort, no-recompute, acyclic render receipt, report parity and successor-lineage gates. Keep release evidence distinct from product acceptance. | Permanent V6 contract checker plus all local gates and exact-head CI/Preview; founder acceptance recorded separately; `main`/Production remain unchanged without explicit authority. |
...
4. Report/dashboard values recomputed from map state instead of read from a frozen DecisionRecord, or any parent DecisionRecord containing its later render artifact/receipt hash.
...
| `WorkspaceAnalysisRun`, `WorkspaceComparisonSet`, `WorkspaceReport` | Store typed DecisionRecord/snapshot/DecisionRenderReceipt refs; remove new `unknown` payload writes. |

## Risks and unresolved decisions

| Question | Required decision before implementation |
| --- | --- |
| Canonical JSON | Approve the specified JCS-compatible canonicalization and exact single excluded top-level hash field for all four contracts. Snapshot truth is locale-independent; render locale is receipt-bearing. |
