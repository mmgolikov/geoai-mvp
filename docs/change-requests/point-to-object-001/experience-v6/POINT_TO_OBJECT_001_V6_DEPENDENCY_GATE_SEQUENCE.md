...
`baseline -> contracts -> adapters -> GeoContext -> Scenario Registry -> DecisionRecord -> shortlist/compare -> Create/evaluate -> dashboard/report/Project Hub -> protected persistence -> exact-head Preview -> separate release decision`
...
3. No report before DecisionRecord; report rendering cannot execute analysis.
...
6. No provider output reaches UI/ranking/model without normalization, rights and source receipts.
7. No Production/main action is implied by a green Preview.
...
| **G1 — contract acceptance** | Product + Architecture + Data + QA | G0 complete; this package reviewed | Approve field semantics, canonical JSON/hash profile, subject/snapshot granularity, evidence classes, context sections, registry lifecycle, DecisionRecord lineage and non-authorizations. | Approved versioned schemas/docs; named open decisions resolved or explicitly deferred. | No V6 runtime implementation; V5.1 remains isolated candidate. |
| **G2 — executable contract kernel** | Engineering + QA | G1 approved | Implement TypeScript DTOs, strict parsers, JSON Schema compilation, JCS/hash utilities, semantic reference/graph validators, positive/negative fixtures and adapter interfaces. Do not change UI behavior yet. | Schema parity, canonical hash/tamper, graph/cycle, locale invariance and bounded payload tests pass. | Fail closed; do not persist or render invalid records. |
| **G3 — V5.1 to GeoContext adapter** | Data + Engineering | G2 green | Normalize `LiveMapSelection`, live object evidence, Find coverage and AOI context into GeoContextSnapshot V1. Preserve observed/calculated distinction, coverage/freshness gaps, conflicts, attribution and proof limits. | Dubai/Singapore object and AOI fixtures show byte-stable hashes and parity with existing source facts; source unavailable/partial/capped/ambiguous adversarial cases pass. | Return partial/blocked snapshot with gaps; do not fall back to demo facts. |
| **G4 — Scenario Registry candidate** | Product + Research + Data + Engineering | G2 green; G3 fact-key catalogue available | Migrate ten current scenario IDs and product roles. Define business questions, mode bindings, context profiles, methods, templates and validation requirements. Preserve current supported/partial/unsupported honesty. | Registry hash/reference/role/mode/method/template validators pass. First end-to-end fixture: `b2b_redevelopment_selected_aoi@1.0.0`. Ranking remains blocked unless separately approved. | Unsupported/under-evidenced mode is disabled with named requirements. |
| **G5 — DecisionRecord and no-recompute path** | Engineering + QA | G3 and G4 green | Implement immutable records and successor lineage. Adapt Analyse/focused analysis to reuse exact snapshot; add browser-local store adapter; bind dashboard/report refs without yet porting old screens. | Find/Analyse/follow-up fixture chain resolves all hashes; refresh creates new snapshot/record; tampered/cross-scope/cyclic references fail. | No record, no downstream decision UI/export. |
...
| **G9 — unified dashboard/report/Project Hub** | Product + Design + Engineering + Documentation + QA | G5 complete; G6 and/or G8 provide real records | Adapt V5.1/earlier dashboard and print primitives. Bind to DecisionRecord templates, source/time/coverage/confidence/gaps and validation tasks. Project Hub indexes immutable refs. | Dashboard/report byte/reference parity; no method calls during render; RU/EN truth invariance; print, accessibility, responsive and unknown-project fail-closed tests. | Do not export if required report fields/gates fail; dashboard may show named gaps. |
...
1. implement all three V1 parsers/hash/semantic validators;
...
5. issue a report render manifest from the same record, without exporting protected data or calling analysis again;
...
- source rights, coverage, freshness and known gaps;
