...
- Independent AI/Create runtime support is integrated locally as `4b8253f` from source `d1ea1d5244199a2ec307e6c3cf5f05a0857b528b`. Both Production flags and the existing server key are required; no hosted flag has been set. Source endpoints remain Preview-only pending the separate approval card. Create now additionally enforces 20 attempts per ten minutes within one warm process, not across all deployments or instances.
