...
## Exact 7c0bf61 Preview acceptance and final maintenance

- Ready protected Preview `dpl_4KWb98Za2sM7hqjZtiVXwFrUrKPb`, `https://geoai-jajjrzih9-geoaidev.vercel.app`, commit `7c0bf6172d53f05e0b887d854a44d34dbae30cc5`: Dubai EN and Singapore RU live context/analysis passed again, as did live Create with the actual browser result parser. Three-operation API estimate: USD 0.0341693.
- A separate real browser generation returned a six-block civic campus, 3–8 levels, 28% coverage and two alternatives containing L-shaped, U-shaped, chamfered and rectangular footprints. It appeared on the map and saved to device-local Projects. Model: `gpt-5.6-sol`, medium; one provider attempt; estimated USD 0.011068. The original buildings can be shown again without regeneration. Mouse/keyboard delivery by the automation tool was unreliable on this page; DOM activation of visible controls was used for this live flow. Real-input responsive controls are independently covered by the controlled Playwright checks.
- Alternative B selection and Projects reopening preserved artifact ID `artifact-4f6d3c2a-d94f-4e3c-97f4-cbc641897f14`, hash `2866020e247e044fe0a2a3ccfb18a6b7895e54d706d7123fd2399a3302951947` and B selection. Create resource count remained two (challenge GET plus one generation POST) throughout: no repeat provider call.
- Search/suggestions passed. One Find request returned the application's bounded, retryable Overpass timeout; the single control repeat passed search, suggestions, Find (three results) and area context (six mapped objects). This is observed availability with a retained failure receipt, not a guarantee of uninterrupted public-source service.
- CI `34045753861` passed its database job but failed six storage-dependent browser scenarios whose fixtures still used anonymous identity. Maintenance signs in through the existing demo UI before those save/reopen/recovery tests; it does not relax runtime identity isolation or Auth controls. The real-producer Projects regression is additionally wired into CI.
- Real reopening also exposed a stale editor snapshot: the generated result was correct, but the saved draft key preceded successful generation and incorrectly showed an unapplied-change state. The bounded correction passes the committed editor snapshot directly with the successful result before saving. The browser regression must verify Already generated before and after B/reopen with no new Create calls. Final successor CI and Preview acceptance remain required before main.
- While maintaining those tests, direct-navigation coverage exposed an additional real Analyse race: its mount effect could start before AuthProvider resolved the existing browser identity, capturing a null save destination. The initial restore/request now waits for session resolution; resolved anonymous visitors still retain public analysis. Tests must preserve direct-navigation coverage after the existing allowed demo login path rather than hide this race with SPA-only navigation. No Auth redirect allowlist or authorization rule changes are included.
