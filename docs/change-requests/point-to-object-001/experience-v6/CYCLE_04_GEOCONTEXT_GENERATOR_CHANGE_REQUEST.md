...
Real-source acceptance amendment, 2026-09-06: the first published adapter rejected both Burj Al Arab (Q62939, revision 2532273591) and Marina Bay Sands (Q548679, revision 2518380772) because its global 0.0001-degree ceiling excluded their standard one-arcsecond P625 representation. Primary [Wikibase DataModel](https://www.mediawiki.org/wiki/Wikibase/DataModel#Geographic_locations) and [Wikidata datatype documentation](https://www.wikidata.org/wiki/Help:Data_type#Globe_coordinate) distinguish coordinate representation resolution from measured positional accuracy. Main approves a bounded correction: maximum supported resolution one arcsecond; unknown, zero and coarser precision still rejected. Assuming rounding to the declared step, use a half-cell diagonal budget with a conservative 112,000 metres per degree on both axes. Polygon interior clearance can absorb this representation budget; exterior distance plus the budget must remain within the unchanged 20 m tolerance. Node/complex distance plus the budget must remain within the unchanged 250 m ceiling. Include hole/concavity and near-ceiling counterexamples. This quantization assumption is not a source accuracy certificate; exact QID/type/country/rank gates, linked-entity scope and prohibition on Create/footprint metric substitution remain intact. Capture real-source observations separately from synthetic fixtures and do not call either live AI acceptance.
