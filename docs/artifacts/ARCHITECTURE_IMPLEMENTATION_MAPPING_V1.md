...
| ERD-001 | Declared Supabase migration source model for organization/project/evidence entities | ordered `supabase/migrations/*` set; `src/lib/db/schema-readiness.ts` | Source mapping does not prove fresh-chain applicability or the live applied schema; current Production is not configured for durable Supabase |
...
| Persistence schema | Ordered Supabase migrations | Supabase/PostGIS foundation | migration SQL + schema readiness table list | Present as source; ordered-chain/applied-column verification remains open; current Production connection absent |
