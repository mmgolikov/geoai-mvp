...
Reconcile legacy and current table definitions into an upgrade-safe canonical chain. The candidate chain now includes containment, identity, source custody, rebuilt Auth/Admin/client/project activation, FK-index hardening and forward lifecycle remediation. Treat all six as rehearsal-only until the development upgrade/drift plan is separately approved and proven.

Current implementation: the first ten live-ledger files remain hash/byte pinned; non-ledger drafts are quarantined. Isolated Free rehearsal `bkmfcjzalcvdsdvyxpgi` now carries all six candidates and the owner Data API operator. Hosted identity/source, activation and lifecycle-remediation suites pass `183/183`; all test users roll back. PostgREST exposes only the 14-RPC `api` schema and HTTP denies `public`; all 29 GeoAI domain tables have RLS and 39 added indexes reduce uncovered domain FKs to zero. Remediation closes invitation expiry rollback/lock order, bootstrap provenance, temporary-ban status drift and the unsafe shared aggregate cursor. Two independent hosted sessions passed the table-level organization→project→invitation create/accept and create/revoke lock-order regression without deadlock and rolled back cleanly. The [receipt](SUPABASE_AUTH_REHEARSAL_RECEIPT_2026_07_16.json) records its RPC/HTTP limitation. DB-01 remains open for authenticated RPC concurrency, resource-specific Admin pagination, real HTTP Auth/MFA/browser/Storage personas and the separate development upgrade/drift/apply decision.
...
Current implementation: exact allowlisted development/rehearsal targeting; effective-mode-gated browser/server SSR clients; middleware claim refresh; PKCE registration/callback; safe session/logout; TOTP enroll/challenge/verify/remove with explicit factor selection, pending-factor cleanup and immediate post-unenroll refresh; and AAL2-gated same-origin Admin/Onboarding APIs are implemented locally. Auth-cookie mutations keep the origin guard outside active Auth mode. The Admin surface calls only `api.organization_admin_snapshot/create_* /revoke_invitation/set_*`; the raw invitation is returned once in a URL fragment, moved into a short-lived HttpOnly same-site cookie for sign-in, then cleared after success, while only its SHA-256 hash reaches the RPC. Before `api.current_profile()`, UUID `claims.sub` must equal canonical `auth.getUser().id` and both anonymous markers must be explicitly false. AUTH-01B still separately validates exact project/action through `api.current_project_access()` and bounded `api.current_source_releases()`. Product repositories and hosted runtime-persona readiness remain false. Static/build/demo-mode HTTP evidence passes; owner-confirmed ENV-01 evacuation permits a fresh hard-mode rehearsal Preview, while real hosted Auth/MFA/RLS/IDOR/Admin personas remain open until that exact deployment is verified.
