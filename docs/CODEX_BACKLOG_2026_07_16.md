...
- retain the locally proven compact 64 KB/48 KB source projections, including `/api/external-data/sources`, and enforce response-byte/API latency budgets in exact-head CI/Preview against the old 133–158 KB baseline;
...
- the generated lifecycle/successor manifest, clickable archive sidecar and CI gate remain complete and derived (current evidence: 129 in-scope Markdown documents, 12 active, 117 non-active/generated, zero unclassified, including `docs/artifacts`); no historical hand count such as 80 becomes a contract;
