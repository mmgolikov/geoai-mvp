...
The responsive/keyboard candidate keeps the same isolated runner boundary and adds three fixed viewport contracts: 1440×900 desktop, 834×1112 tablet and 390×844 mobile. Landing and login must retain one landing `h1`, both bounded entry actions, the unauthenticated profile affordance and no horizontal overflow. On mobile, the three primary entry controls must meet the chosen 40px application baseline, and a Tab/Enter-only journey must reach the mock demo, Workspace, authenticated profile affordance and `/profile`. These assertions are UX checks only; they do not grant server access, inspect live customer data or replace Axe/Lighthouse and the deeper analysis/report/print keyboard suite.
