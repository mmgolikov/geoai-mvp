...
The lower access decision is authoritative, public-demo server mutations/generation are denied, and affected ID routes resolve stored project scope before authorization. The inactive permanent-user boundary requires UUID `claims.sub` exactly equal to canonical `auth.getUser().id` plus explicit claims/user `is_anonymous === false` before profile RPC; mismatch is 401, anonymous identity 403 and ambiguity fails closed. AUTH-01B then verifies exact caller/project scope and maps explicit source DTOs. `client_viewer`, malformed/cross-tenant rows, base-table reads, service-role clients and public caching are excluded. All readiness flags remain false, no route consumes the facade and repositories stay disabled until live personas pass.
