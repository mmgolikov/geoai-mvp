...
Released baseline: `main` and `release/production` `7f323c4227f2409f3fe2d4d68be48a30176f4e2a` (merged PR #113), Production `dpl_4yBHCo1eZ7N6GYQWGAg1EdQGwFTE` (`READY`) at https://geoai-mvp.vercel.app, rollback `unverified`, stage `public_demo_prototype`
...
Navigation: [Documentation Index](DOCUMENTATION_INDEX.md) · [Current Release State](CURRENT_RELEASE_STATE.md) · [Product Baseline and Readiness](PRODUCT_BASELINE_AND_READINESS.md) · [External Authority Registry](EXTERNAL_AUTHORITY_REGISTRY.json) · [Data Strategy](data-strategy.md) · [Roadmap](roadmap.md)
...
| Database | Supabase/Postgres/PostGIS development foundation; exact ten-entry historical development ledger reconstructed; local custody lists seven pending candidates | The 2026-07-16 rehearsal receipt recorded the first six candidates, hosted SQL `183/183`, API-only PostgREST and rollback-only table-level invitation concurrency. Current hosted schema/ledger/rows/RLS/policies/PostgREST/Storage state is unverified; real email/phone/Admin/Storage personas, development apply and Production remain blocked |
...
## Released Product System v3.2.2 boundary

PR #113 released the Product System v3.2.2 correction. Its Production runtime is the current visual/product baseline. The historical CR 10.02 Product System v3.2.1 work introduced a scoped `.geoai-v32` semantic token layer and adapted `TopNavigation` / `ProductNavigation` / `AccessStatusBadge` into one 64 px Product shell. Historical receipt `1819:11` records the component-only accessibility correction: enabled inactive text is `#606f83`, while disabled text is `#667587`; no Axe/WCAG exception is permitted. `src/design-system/tokens.ts` remains the typed canonical token source; Tailwind consumes it, while `app/product-system-v3.css` exposes matching variables only under the shell/primitives scope. Geist is loaded through `next/font` only in that scope, so non-shell route bodies retain their existing typography and presentation.

The approved 32 px identity is a repository-owned historical Figma export with a pinned SHA-256. Button, StatusChip, SegmentSwitch and ValidationCaveat are typed migration primitives, while existing page-local controls and `SafeBadge` remain in place until separately approved body migrations. Static drift and Playwright/Axe contracts preserve token values, navigation, responsive targets and the no-body-migration boundary. No current Figma read proves exact PR #113 parity, so Figma remains `partial`; Production is authoritative. Draft PR #143 is excluded and cannot be used as a design source. This boundary changes no session, authorization, API, persistence or source trust decision and authorizes no new release action.
