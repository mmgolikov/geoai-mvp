...
- [x] Static route-gate contract requires `/workspace`, `/projects`, `/explore` and `/profile` to wait for resolved browser session state in `supabase_auth`, use only the bounded login continuation for resolved anonymous users, preserve `demo_public` and fail closed in `disabled`. **This is client-rendered UI containment, not server authorization; fresh exact-head CI/Preview and rendered-browser redirect/session-restoration evidence are pending.**
