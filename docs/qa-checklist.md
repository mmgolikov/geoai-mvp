...
This checklist uses the current externally verified PR #113 public-demo baseline at `7f323c4227f2409f3fe2d4d68be48a30176f4e2a` on `main` and `release/production`, Production `dpl_4yBHCo1eZ7N6GYQWGAg1EdQGwFTE` (`READY`) at https://geoai-mvp.vercel.app, rollback `unverified` and stage `public_demo_prototype`; see [Current Release State](CURRENT_RELEASE_STATE.md) and the [External Authority Registry](EXTERNAL_AUTHORITY_REGISTRY.json). Use only built-in synthetic fixtures and do not enter/upload confidential, regulated, sensitive or client-protected AOIs, CSV, GeoJSON, filenames, evidence or dynamic package data. Protected persistence and real sources remain blocked.
...
## Production baseline recovery gates

- [x] Active release-fact documents and `EXTERNAL_AUTHORITY_REGISTRY.json` agree on PR #113, exact SHA, exact Vercel deployment, `READY` and `public_demo_prototype`; an adversarial tuple mismatch fails the fixture contract.
- [ ] Commit B acceptance requires production and full dependency audits to report zero moderate/high/critical findings on a supported Node version; local working-tree receipts do not replace an exact-head hosted gate.
- [x] `npm run test:user-profile` is a Quality Gate step and checks the wrapper label plus the visual authenticated-state marker.
- [x] The exact mandatory caveat is present on comparison, printable comparison and Project Hub surfaces and enforced by the data-honesty guard.
- [x] The retired self-mutating legacy workflow parses as valid YAML, has read-only permissions and contains no checkout, commit or push path.
- [x] Production visual smoke is recorded separately as `observed`; it does not replace exact-head local/browser or protected-persona E2E.
...
- [x] The active product flow has no MFA enrollment, challenge, recovery, factor-management route or AAL2 redirect. Admin still requires a permanent non-anonymous Supabase identity and authorized role; the browser-only mock demo cannot satisfy this boundary.
- [x] Public Preview email-link and phone OTP calls set `shouldCreateUser: false`; static source checks reject any public `shouldCreateUser: true` or `auth.signUp()` path, and the permanent Chromium contract intercepts both `/auth/v1/otp` requests to assert `create_user: false` without contacting Supabase.
- [x] Public Auth copy is existing-user-only and does not promise automatic registration. New-user onboarding requires a separately approved invitation/server policy; password sign-in and the browser-local demo remain unchanged and non-authoritative.
- [x] Repository-pending migration `20260716213214_simplify_auth_remove_mfa_requirement.sql` defines `require_permanent_identity()`, uses exact permanent non-anonymous error/comment language and keeps `require_aal2()` only as a compatibility wrapper. It does not assert verified email or phone ownership; its current hosted application state is unverified for every target.
...
- [x] Landing `View demo` enters `/login?next=/workspace&intent=demo`; commercial request actions enter `/request-access`, where the five bounded fields generate an unsent React-memory brief with no network mutation or browser storage. Legacy Login request intent redirects to `/request-access`. Saved demo sessions continue automatically, and the authenticated profile icon opens `/profile`. **This separation does not create a request backend or real-user Auth authority.**
- [x] Static route-gate contract requires `/workspace`, `/projects`, `/explore` and `/profile` to wait for resolved browser session state in `supabase_auth`, use only the bounded login continuation for resolved anonymous users, preserve `demo_public` and fail closed in `disabled`. Exact functional head `77ac593b51d43a62ddc89656dbae735378cab69f` passed Quality Gate `29579739837`; READY Preview `dpl_6Er5tTEesM2V6RA7ZQD8eR5VYJpQ` returns only the restoration shell and no product content for all four route HTML responses. **This is client-rendered UI containment, not server authorization; later exact Playwright suites prove mock-session redirect/restoration only, while real-user evidence remains pending.**
...
- [ ] Real snapshots have explicit tenant/visibility and custody; nullable project scope is not treated as public. **Current status: the historical rehearsal receipt recorded schema-contract apply/SQL testing; current hosted physical state is unverified, and development/Production apply, real source personas, rights and trusted-worker evidence remain blocked.**
...
- [x] Released static check: public-demo upload, AOI, report, analysis-run and comparison state remains browser-only; Vercel server-local fallback is disabled and seeded reports are immutable server authority. **This is browser-state isolation, not protected persistence.**
...
- [ ] All seven pending migrations pass target-derived development upgrade/drift replay plus real Auth/RLS/source personas before development apply. **The 2026-07-16 receipt recorded clean rehearsal-only replay and 183/183 hosted SQL evidence for the first six plus the owner API operator. Local manifest state is 1 pre-ledger + 10 immutable historical ledger + 7 pending with `liveApplyReady:false`; current hosted application state is unverified.**
...
- [x] `npm run supabase:migrate:check` enumerates seven pending local migrations and requires canonical-chain, security-surface, identity/authorization, SOURCE-01 custody, Auth/Admin activation, FK-index, lifecycle-remediation, no-MFA permanent-identity compatibility and Data API operator static checks. **It deliberately reports development/live apply readiness false and makes no current hosted application claim.**
- [x] The 2026-07-16 receipt recorded historical migrations plus the first six candidate migrations replayed on isolated Free rehearsal, 183/183 hosted SQL personas, API-only PostgREST probes and rollback-only table-level invitation concurrency. **Current hosted physical state is unverified. The seventh migration requires separate exact-target approval and new personas. This is not authenticated RPC/HTTP concurrency, development drift certification, real email/phone/Storage E2E or Production authorization.**
...
- [ ] The development Data API is disabled or exposes only an intentionally minimal `api` schema; minimum grants and RLS are both verified. **The historical rehearsal receipt passed this boundary; current rehearsal/development physical state is unverified.** See the [containment runbook](SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md).
- [x] The 2026-07-16 rehearsal receipt recorded direct `public` denial with `PGRST106`, base-table denial in `api` with `PGRST205`, and anonymous `api.healthcheck()` HTTP 200. **Current hosted PostgREST state is unverified.**
...
- [x] Historical CR 10.02 evidence proves the scoped Product System v3.2.1 shell and accessibility correction; PR #113 subsequently released the Product System v3.2.2 correction. **Production is now the visual/product baseline. Figma parity remains partial, page-body redesign is not authorized, and draft PR #143 is excluded.**

## Responsive Checks

- [x] Exact-head Playwright coverage passes for 1440px desktop, 834px tablet and 390px mobile landing/login layouts, horizontal overflow, primary mobile entry targets and a keyboard-only demo → Workspace → profile path: head `e203e895406817497f339fccf1d04da377a7bc65`, Quality Gate `29584919107`, Chrome `5/5` in `41.5 s`. It does not cover Axe/Lighthouse or deep product/print flows.
- [x] Homepage renders on desktop in the exact-head Playwright landing contract above.
