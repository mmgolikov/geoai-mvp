...
- [x] Permanent-user evidence requires UUID `claims.sub === auth.getUser().id` and explicit claims/user `is_anonymous === false` before profile RPC; mismatch maps to 401, anonymous identity to 403 and ambiguity fails closed. **Inactive: readiness false, no route/repository activation, live HTTP/JWT/RLS personas pending.**
