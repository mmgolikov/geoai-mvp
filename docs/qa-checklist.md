...
- [x] TOTP enrollment/challenge/removal requires explicit factor selection, exposes cleanup for pending factors and refreshes the session immediately after unenrollment; AAL2 gates Admin RPCs.
...
- [x] `npm run supabase:migrate:check` enumerates six pending migrations and requires canonical-chain, security-surface, identity/authorization, SOURCE-01 custody, Auth/Admin activation, FK-index, lifecycle-remediation and Data API operator static checks. **It deliberately reports development/live apply readiness false; all six candidates remain rehearsal-only.**
- [x] Historical migrations plus all six candidate migrations were replayed only on the isolated Free rehearsal. Hosted SQL personas pass 183/183; API-only PostgREST is proven by an allowlisted health RPC and negative `public`/base-table probes. Two independent backend sessions also pass rollback-only table-level invitation create→accept/revoke lock-order checks. **This is not authenticated RPC/HTTP concurrency, development drift certification, real HTTP Auth/MFA/Storage E2E or Production authorization.**
