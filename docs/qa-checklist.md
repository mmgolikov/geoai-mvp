...
- [x] Pending migration `20260716213214_simplify_auth_remove_mfa_requirement.sql` defines `require_permanent_identity()`, uses exact permanent non-anonymous error/comment language and keeps `require_aal2()` only as a compatibility wrapper. It does not assert verified email or phone ownership and remains unapplied everywhere.
...
- [x] `npm run supabase:migrate:check` enumerates seven pending migrations and requires canonical-chain, security-surface, identity/authorization, SOURCE-01 custody, Auth/Admin activation, FK-index, lifecycle-remediation, no-MFA permanent-identity compatibility and Data API operator static checks. **It deliberately reports development/live apply readiness false. The first six candidates are rehearsal-only; the seventh is unapplied everywhere.**
...
- [x] CR 10.02 locally proves one scoped Product System v3.2.1 shell at 1440×900, 1024×900, 768×1024 and 390×844: exact 64 px header, active routes, approved identity, 40/44 px targets, deterministic double captures, no shell/body overlap or horizontal overflow, keyboard focus, Escape/outside-click/route closure and zero serious/critical Axe findings. The permanent primitive harness covers exactly 56 states and receipt `1819:11` separates enabled inactive `#606f83` from disabled `#667587` without an Axe exception. A byte-identical 20-image body-only Linux comparison covers Workspace, Projects, Explore, Request Access and Profile. **Exact-head GitHub Quality Gate, immutable artifact, Vercel Preview and founder review remain required before Ready-for-review; page bodies and real-user authorization are not covered by this design gate.**
