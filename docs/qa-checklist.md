...
Exact snapshot receipt: documentation/evidence head `f39dcee18c2601b88ae51e061627925037d1aa77` preserves functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; GitHub Quality Gate run `29497314994`, application job `87617248273` and Supabase replay/pgTAP job `87617248275` succeeded, with clean start/reset and all 71 assertions. Artifact `8374954241` is `geoai-quality-evidence-29497314994`, digest `sha256:e3a5bbfdfb3278e7ed69cb8ab4ba2c8b6137fbbc091064ba38f6b05a5362c73a`. Exact-docs-head Preview `dpl_Gh5btUKs8yzySxvpxGnwXjMkYayK` is READY at `https://geoai-6oirbt1at-geoaidev.vercel.app`. HTTP/API/security and response-size receipts below are not browser-flow evidence; the local headless browser was blocked by a system Unix-socket restriction, so desktop/mobile/keyboard/print checks remain unclaimed.
...
- [x] Candidate static check: the environment-driven access wrapper denies hard mode when Auth/bypass evidence is absent; synthetic membership cannot override the lower decision. **Exact-snapshot application job `87617248273` passes; live HTTP/JWT/RLS personas remain pending.**
...
- [x] Exact-snapshot GitHub CI `database-replay` completes clean start/reset and the 71-assertion pgTAP database/source/Storage-helper plan across all three pending migrations. The 14 red-team additions cover exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and artifact/status-event/ingestion-receipt update/delete immutability; the policy sweep also covers every custody/audit table for `public` or authenticated-applicable policies. **Receipt: run `29497314994`, job `87617248275`, PASS. Upgrade/live/persona/advisor parity remains separate.**
...
- [x] `npm run test:api-contract` passed in exact-snapshot application job `87617248273`; exact-docs-head Preview returns 200 for `/`, `/workspace`, `/explore`, `/api/health` and `/api/platform/activation-status`, 503 for the disabled source pack, and 400 for invalid climate coordinates.
- [x] `npm run test:security-headers` passed in exact-snapshot application job `87617248273`, and CSP, HSTS, `nosniff` and frame `DENY` are present in the exact-docs-head Preview HTTP matrix. **This is HTTP evidence, not browser/a11y evidence.**
...
- [x] `npm run test:source-custody-migration` statically verifies the five custody tables, RLS/direct-grant closure, immutability, tenant/actor FKs and bounded RPC; exact-snapshot Quality Gate run `29497314994` passes.
