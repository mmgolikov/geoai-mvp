...
Functional candidate receipt: content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; GitHub run `29496255541` is partial/overall failure because application job `87613914544` stopped at stale pre-publication documentation current-truth; ephemeral Supabase replay/pgTAP job `87613914543` passed clean start/reset and all 71 assertions. No current artifact is claimed; a post-publication all-green exact-head Quality Gate and artifact are required. Exact-functional-SHA Preview `dpl_EaXsfjW5y2WUkCoXoeTYmGiD1DiY` is READY at `https://geoai-ltagjva86-geoaidev.vercel.app`. HTTP/API/security and response-size receipts below are not browser-flow evidence; the local headless browser was blocked by a system Unix-socket restriction, so desktop/mobile/keyboard/print checks remain unclaimed.
...
- [x] Candidate static check: the environment-driven access wrapper denies hard mode when Auth/bypass evidence is absent; synthetic membership cannot override the lower decision. **The functional-content application-job step passed before the stale-documentation gate; live HTTP/JWT/RLS personas and an all-green post-publication run remain pending.**
...
- [x] Exact-functional-content GitHub CI `database-replay` completes clean start/reset and the 71-assertion pgTAP database/source/Storage-helper plan across all three pending migrations. The 14 red-team additions cover exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and artifact/status-event/ingestion-receipt update/delete immutability; the policy sweep also covers every custody/audit table for `public` or authenticated-applicable policies. **Receipt: run `29496255541`, job `87613914543`, PASS. The overall run is partial/failed; post-publication all-green Quality Gate/artifact and upgrade/live/persona/advisor parity remain separate.**
...
- [x] `npm run test:private-cache-boundary` proves all 18 manifest-classified project GET routes plus Auth session use `private, no-store, max-age=0` and `Vary: Authorization, Cookie`; only the explicit immutable package seed branch may be public. **The functional-content application-job step passed before the stale-documentation gate; protected live-persona evidence and an all-green post-publication run remain pending.**
...
- [x] `npm run test:api-contract` passed in the functional-content application job before the stale-documentation gate; exact-SHA Preview returns 200 for `/`, `/workspace`, `/explore`, `/api/health` and `/api/platform/activation-status`, 503 for the disabled source pack, and 400 for invalid climate coordinates. **An all-green post-publication run remains pending.**
- [x] `npm run test:security-headers` passed in the functional-content application job before the stale-documentation gate, and required security headers are present in the exact-SHA Preview HTTP matrix. **An all-green post-publication run remains pending; this is HTTP evidence, not browser/a11y evidence.**
...
- [x] `npm run test:source-custody-migration` statically verifies the five custody tables, RLS/direct-grant closure, immutability, tenant/actor FKs and bounded RPC. **A complete post-documentation all-green exact-head application gate remains pending; partial run `29496255541` is not cited as success.**
...
- [x] Historical migrations plus the canonical reconciliation and three pending migrations pass an exact-functional-content ephemeral clean replay. **Upgrade replay, development drift, live personas and owner-controlled apply remain uncertified.**
