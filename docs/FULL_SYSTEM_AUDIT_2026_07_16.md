...
| Unreleased audit candidate | Functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; evidence snapshot `f39dcee18c2601b88ae51e061627925037d1aa77`; run `29497314994` success; DB job `87617248275` 71/71 PASS; Preview `dpl_Gh5btUKs8yzySxvpxGnwXjMkYayK` READY | Upgrade replay, live apply/Data API containment, credential evacuation, live JWT/Storage/source personas and browser quality remain required |
...
1. **The historical migration/identity/source-custody chain has an exact-snapshot ephemeral clean-replay receipt but is not upgrade/live certified.** The repository preserves the exact ten-entry live ledger by version/bytes/MD5, quarantines non-ledger SQL, records the pre-ledger healthcheck reconciliation and stages upgrade-safe tenant constraints, private helpers, exact RLS policy templates and an `api` allowlist. Documentation/evidence snapshot `f39dcee18c2601b88ae51e061627925037d1aa77` preserves functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; run `29497314994`, DB job `87617248275`, passed clean start, deterministic reset and all 71 assertions. The 14 red-team additions cover exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and artifact/status-event/ingestion-receipt update/delete immutability; the policy sweep covers all custody/audit tables for `public` or authenticated-applicable policies. Local Docker remains unavailable. Upgrade replay/parity, live Data API/ACL containment, schema drift/advisor parity and real JWT/HTTP/Storage/source personas remain mandatory [DB-01](CODEX_BACKLOG_2026_07_16.md#db-01--canonical-migration-replay-and-rls-evidence) evidence.
...
12. Documentation/evidence snapshot head `f39dcee18c2601b88ae51e061627925037d1aa77` passed Quality Gate run `29497314994`: app job `87617248273` succeeded; DB job `87617248275` recorded `Files=1, Tests=71`, `All tests successful`, `Result: PASS`; artifact `8374954241` has digest `sha256:e3a5bbfdfb3278e7ed69cb8ab4ba2c8b6137fbbc091064ba38f6b05a5362c73a`. Upgrade replay, live HTTP/JWT/Auth/source/Storage personas, adversarial uploads, browser accessibility/performance budgets and immutable action pinning remain missing.
...
- **GO** for continued public-demo development and the evidenced non-Production audit Preview on documentation/evidence snapshot `f39dcee18c2601b88ae51e061627925037d1aa77`; this does not authorize Production or external activation.
...
The first published Draft PR head and Preview passed their then-current checks, but final red-team work superseded that evidence. Documentation/evidence snapshot head `f39dcee18c2601b88ae51e061627925037d1aa77`, over functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`, passed Quality Gate run `29497314994`; app job `87617248273` and Supabase job `87617248275` succeeded, and artifact `8374954241` preserves the evidence. Vercel Preview `dpl_Gh5btUKs8yzySxvpxGnwXjMkYayK` is READY on the exact snapshot head. Its explicit HTTP matrix returned the expected 200 routes, 503 disabled/empty source pack, 400 invalid climate coordinates and CSP/HSTS/nosniff/frame-DENY headers; compact route sizes are recorded in [Current Release State](CURRENT_RELEASE_STATE.md). Browser/mobile/keyboard/print evidence remains unclaimed because local headless launch is blocked by a system Unix-socket restriction. Production remains PR #87, and none of these receipts applies Supabase, rotates credentials or activates Auth, Storage or providers.
