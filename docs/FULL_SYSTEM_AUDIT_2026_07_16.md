...
Current unreleased Auth product decision: one simple email-or-phone sign-in screen, no MFA, no user-entered invitation token, and a browser-only mock demo using `demo@geoai.space` / `111111`. Email uses a Supabase sign-in link; phone uses SMS OTP only after an owner configures an SMS provider. A seventh migration replaces Admin AAL2 with verified permanent identity, but it is prepared only and has not been applied to rehearsal, development or Production.
...
2. **Permanent-user identity and AUTH-01B are implemented locally but not activated or persona-certified.** Before profile RPC, UUID `claims.sub` must equal canonical `auth.getUser().id`, and claims/user anonymous markers must both be explicitly false; mismatch is 401, anonymous identity 403 and ambiguity fails closed. Exact-target SSR/PKCE/session/logout, email sign-in links, phone SMS OTP, automatically staged invitation links and verified-identity Admin RPC routes are present and pass static/build/local HTTP smoke. MFA is deliberately absent from the current product flow. The mock demo is browser-only and cannot satisfy protected API authorization. The exact-project Product facade remains disconnected, readiness false, Product repositories disabled and live hosted HTTP/JWT/RLS/IDOR personas absent.
...
- a complete 88-handler access manifest with 51 project-scoped decisions, two verified-identity organization-admin handlers, three identity-mutation handlers and explicit public-demo classifications;
...
- a permanent-user identity gate requiring matching UUID claims/user subjects and explicit non-anonymous evidence before profile RPC; effective-mode-gated exact-target SSR/PKCE/session/logout, one email-or-phone login screen, automatic short-lived HttpOnly invitation handoff and verified-identity Admin RPC routes; plus a browser-only mock demo and a disconnected exact-project AUTH-01B Product facade whose persona-readiness flags remain false;
