...
| Candidate Auth browser contract | Focused Playwright flow is authored and CI-wired for bounded guest continuation, browser-only mock-demo restoration/direct navigation and logout re-gating. It constructs a fake public-key-shaped value at runtime, uses no real credential or hosted identity and has no Supabase write path. Exact workflow evidence is pending; real Auth/RLS/Admin personas remain open |
