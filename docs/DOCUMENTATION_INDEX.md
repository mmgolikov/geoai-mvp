...
| Responsive/keyboard browser candidate | The focused Playwright command now discovers five tests: desktop/tablet/390px landing-login layout and overflow checks plus a 390px keyboard-only demo → Workspace → authenticated profile journey and primary-entry target-size baseline. Static contract/discovery/TypeScript pass locally; actual Chrome CI, Axe/Lighthouse, deep product/print journeys and real email/phone personas remain pending |
