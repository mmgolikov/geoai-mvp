...
The newest local-only profile increment adds `/profile`, full-name/region/contact details, a browser-local avatar, registered-email/password actions and default B2B/B2C role propagation into Workspace and Projects. It does not use user-editable preferences for authorization, does not enable verified sign-in-phone mutation or protected Storage, and does not change any live Supabase/Vercel Production state. Its local build and HTTP checks are not hosted or rendered-browser evidence until a fresh exact-head PR Preview exists.
