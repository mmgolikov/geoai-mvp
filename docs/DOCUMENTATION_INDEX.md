...
| Supabase operator prerequisite | [Data API Containment Runbook](SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md) | Live development exposure, owner decision and evidence required before apply; draft/not executed |
...
| Confluence synchronization | [Confluence sync map](CONFLUENCE_SYNC_MAP.json) | Page-ID to role, authority and successor map for atomic Hub/document updates |
...
| Development Supabase | Ref `pphdqkurxneyagvnnjdt`, separate from Production; exact ten-entry ledger reconstructed; three containment/identity/source-custody migrations unapplied; Data API exposure remains; local replay contract configured but not execution-certified |
| Maturity | Not Production-ready and not pilot-ready |

The full-system audit branch is published as Draft PR #97 but remains unreleased. Exact candidate evidence is volatile and belongs in the PR and Confluence evidence register. It does not authorize a merge, Production deployment, Supabase apply, secret change, Auth activation, real source connection or geometry publication.

Candidate-only current truth: anonymous infrastructure routes are static/sanitized; `/api/known-limitations` is a static reviewed catalog; public source manifest/sources/status use bundled `compact_public_v1` (`contractVersion: 1.3`, `manifestVersion`/`version: 1.6`, `liveRegistryIncluded: false`). The final local production build measured data-sources 5,063 B, readiness 4,467 B, manifest 19,448 B, sources 5,063 B, status 8,158 B and source-lineage 4,352 B within 48/64 KB budgets. Lazy Workspace result surfaces reduced the same local-build First Load JS measure from 252 kB to 218 kB (approximately 13.5% gzip); this is not exact-Preview/Core Web Vitals evidence. Public-demo artifacts are project-scoped browser-local; protected controls are disabled before fetch; explicit data lookups fail closed and invalid Workspace URLs clear/reset visibly; `/demo` redirects 307 to `/workspace`. The AOI validator rejects non-finite/out-of-WGS84/wrong-arity coordinates and unsupported antimeridian crossings in an 11-persona adversarial contract. DB/AUTH/SOURCE staging includes exact ledger checks, three fail-closed containment/identity/source-custody drafts, SSR cookie transport, exact route actions and secret/operator guards. AUTH-01B adds a cookie-only exact-project read facade around caller-bound `api.current_project_access()`, the shared role kernel and bounded approved source-release DTOs, with no `client_viewer`, base-table, service-role or cache path; all readiness flags remain false and no route consumes it. SOURCE-01 defines five RLS-closed custody tables and immutable release/artifact/status/receipt records with tenant/actor FKs; legacy backfill defaults to `restricted`/`registered_unverified`, and its approved-only RPC omits arbitrary quality/lineage JSON, paths, URI and secrets. SOURCE-02 is a pure provider-neutral planner/receipt foundation with an empty registry, no fetch/env/secrets/persistence and Production denied; objective rights/replay/custody/persona/worker/owner/exact-SHA/rate/circuit/credential plus applicable distribution/geometry/imagery gates remain required. The local replay contract pins Supabase CLI `2.109.1`, exposes only `api` on Postgres 17 and defines CI `database-replay` for clean reset plus 57 pgTAP assertions; the operator guard enumerates all three pending migrations and the static source-custody checker is also in CI. The review-only Storage policy uses a narrow role predicate and operation-aware no-listing reads without direct caller joins to protected tables; it remains unapplied. Docker is unavailable locally and exact-SHA GitHub execution remains pending, so this is not replay, Storage or provider-activation evidence. Final exact-head SHA/CI/Preview and remote runtime negative evidence remain pending. The previous candidate head/CI/Preview recorded in the [Full System Audit](FULL_SYSTEM_AUDIT_2026_07_16.md#released-live-development-and-candidate-boundaries) is superseded.
...
The machine-readable [Confluence sync map](CONFLUENCE_SYNC_MAP.json) is the complete synchronization contract for the 25-page active/supporting operational authority set, not for every historical page in the space. Primary authorities are: Hub `98425`; Home `98509`; Delivery `2097153`; Audit `12320972`; Artifact Register `1343521`; Change Log `98732`; Work Packages `1966084`; Risk `98712`; Decisions `98692`; Security `2490398`; Pilot Readiness `2686985`; Governance `294937`; Release Evidence `5242921`; and PR Disposition `5111891`. Product, Engineering, Data/AI, System Analysis, UX/UI, GTM and both roadmaps remain mapped supporting authorities. `CR-DEV8-001` (`12320810`) is historical released evidence and points to Current Delivery State as successor. The other 229 pages remain historical/scoped cleanup inventory under DOCS-01; synchronizing the operational set does not silently certify them as current.
...
- Development Data API operator decision and live evidence: [Supabase Data API Containment Runbook](SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md). It is a draft, has not been executed and is not apply-ready.
