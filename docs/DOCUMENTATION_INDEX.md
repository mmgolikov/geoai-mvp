...
| POINT_TO_OBJECT_001 V5.1 candidate | [V5.1 Change Request](change-requests/point-to-object-001/experience-v5/POINT_TO_OBJECT_001_V5_1_INTERACTION_FIND_PROFILE_CHANGE_REQUEST.md) | Founder-approved isolated Preview implementation scope for Analyse/Find/Create, Photon autocomplete and profile alignment; exact-head receipt, CI, Preview and browser evidence pending; not merged or released |
...
| POINT_TO_OBJECT_001 V5.1 candidate | Branch `codex/point-to-object-clickable-prototype-v1` and PR #147 are the designated delivery path for an isolated Analyse/Find/Create candidate with server-only Photon suggestions, explicit Nominatim lookup and bounded Overpass object/AOI context. It is not merged or Production-active; exact-head CI, protected Preview, browser evidence and a new receipt remain pending. The released source pack stays count-only/disabled, with `main`, Production and hosted Supabase unchanged |
