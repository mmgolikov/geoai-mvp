...
Candidate-only current truth: anonymous infrastructure routes are static/sanitized; `/api/known-limitations` is a static reviewed catalog; public source manifest/sources/status use bundled `compact_public_v1` (`contractVersion: 1.3`, `manifestVersion`/`version: 1.6`, `liveRegistryIncluded: false`). The final local production build measured data-sources 5,063 B, readiness 4,467 B, manifest 19,448 B, sources 5,063 B, status 8,158 B and source-lineage 4,352 B within 48/64 KB budgets. Public-demo artifacts are project-scoped browser-local; protected controls are disabled before fetch; explicit data lookups fail closed and invalid Workspace URLs clear/reset visibly; `/demo` redirects 307 to `/workspace`. DB/AUTH/SOURCE staging now includes exact ledger checks, three fail-closed containment/identity/source-custody drafts, SSR cookie transport, bounded `api` identity/membership/source-release RPCs, exact route actions and secret/operator guards. SOURCE-01 defines five RLS-closed custody tables and immutable release/artifact/status/receipt records with tenant/actor FKs; legacy backfill defaults to `restricted`/`registered_unverified`, only `approved` entries are readable, no provider write path opens, and `client_viewer` cannot read raw release metadata. The local replay contract pins Supabase CLI `2.109.1`, exposes only `api` on Postgres 17 and defines CI `database-replay` for clean reset plus 57 pgTAP assertions; the operator guard enumerates all three pending migrations and the static source-custody checker is also in CI. The review-only Storage policy uses a narrow role predicate and operation-aware no-listing reads without direct caller joins to protected tables; it remains unapplied. Docker is unavailable locally and exact-SHA GitHub execution remains pending, so this is not replay, Storage or provider-activation evidence. Activation evidence flags remain false. Final exact-head SHA/CI/Preview and remote runtime negative evidence remain pending. The previous candidate head/CI/Preview recorded in the [Full System Audit](FULL_SYSTEM_AUDIT_2026_07_16.md#released-live-development-and-candidate-boundaries) is superseded.
