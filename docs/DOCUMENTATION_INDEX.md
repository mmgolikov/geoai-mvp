...
The current working-tree follow-up adds a resolved-session client gate to Workspace, Projects, Explore and Profile in `supabase_auth`: it restores the saved browser session before rendering, sends resolved anonymous visitors to the unified login with a bounded return path, preserves public-demo behavior and fails closed when Auth is disabled. It is not server authorization and does not activate Product repositories, RLS, Storage or real user personas. Fresh exact-head CI and Preview evidence are pending publication.
