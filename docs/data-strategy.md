...
Real source ingestion must use a separate operator/worker plane. Before profile RPC, the inactive AUTH boundary requires UUID `claims.sub === auth.getUser().id` and explicit claims/user `is_anonymous === false`; mismatch is 401, anonymous identity 403 and ambiguity fails closed. AUTH-01B then resolves caller project scope and approved source DTOs only. All readiness flags remain false, no route/repository is active and real personas are absent, so real-source persistence and protected files remain blocked.
...
The separate development Supabase ref `pphdqkurxneyagvnnjdt` still exposes `public` through the Data API; all three review-only migrations are unapplied. Fresh read-only evidence remains 20 public tables/19 RLS, ten applied/zero candidate migrations, zero Auth users, four buckets/zero object policies and 22 public-table `TRUNCATE` grants for each public role. Head `e999c5a07d3ced6c95f2eb44f6a5f03a9c17caea`, tree `73b7c198813d6aede795b8b186bd4d58e741b181`, passed run `29500488408`; DB job `87627894968` passed clean 71/71, a synthetic ledger-prefix rehearsal and a second 71/71. This is not a current-development clone, live-derived upgrade replay, drift, live apply or DB-01 certification. Before any source write, the owner must contain the Data API and prove live personas. Provider writes remain blocked. See the [containment runbook](SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md); it authorizes no live change.
