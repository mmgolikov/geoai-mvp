...
Released baseline: merged PR #113 / `7f323c4227f2409f3fe2d4d68be48a30176f4e2a` on `main` and `release/production` / Production `dpl_4yBHCo1eZ7N6GYQWGAg1EdQGwFTE` (`READY`) at https://geoai-mvp.vercel.app / rollback `unverified` / `public_demo_prototype`
...
Navigation: [Documentation Index](DOCUMENTATION_INDEX.md) · [Current Release State](CURRENT_RELEASE_STATE.md) · [Product Baseline and Readiness](PRODUCT_BASELINE_AND_READINESS.md) · [External Authority Registry](EXTERNAL_AUTHORITY_REGISTRY.json) · [Architecture](architecture.md) · [Roadmap](roadmap.md)
...
The 2026-07-16 isolated Free Auth rehearsal receipt recorded that the SOURCE-01 schema coexisted with the rebuilt and lifecycle-remediated tenant/Auth/Admin model: hosted SQL personas passed `183/183`, the Data API exposed only reviewed `api` RPCs, and direct `public` HTTP access was denied. Current hosted physical state is unverified. No provider is connected in the released Product and no trusted worker exists; the historical evidence authorizes no source fetch, write, scoring or Product claim. [Receipt](SUPABASE_AUTH_REHEARSAL_RECEIPT_2026_07_16.json).
