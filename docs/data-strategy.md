...
The separate development Supabase ref `pphdqkurxneyagvnnjdt` still exposes `public` through the Data API; all three review-only containment, identity and source-custody migrations are unapplied. Before any source registry/release is written there, the owner must either disable the Data API or expose the selected minimal `api` schema, close profile/organization/project identity constraints, and prove anon/authenticated/Storage/source negative personas. The repository's `api`-only Postgres 17 local config, pinned Supabase CLI, three-migration operator guard, static custody checker, 57-assertion pgTAP suite and CI replay job are staged controls, not execution evidence: local Docker is unavailable and exact-SHA GitHub execution is pending. Provider writes remain blocked until a trusted operator/worker write path, rights gates, receipts, rollback and real personas pass. See the [containment runbook](SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md); it authorizes no live change.
