...
The current MVP deliberately has no two-factor authentication. `/login` is the single entry point: email sends a Supabase sign-in link and creates a new user when needed; phone sends and verifies an SMS code after an SMS provider is configured. `/register` and `/mfa` redirect to `/login`, and the callback defaults to `/workspace`. Onboarding never asks the user to paste a token: invitation links are staged automatically, while users without an invitation see a simple workspace/projects start screen.
...
- Current functional/evidence head: `0edf442f7aa59f0fe1f82f26ef6ad7ca9dde7868`, tree `ac23e66e33b8f42ea8e603797b0dc25b2ea87022`; Quality Gate run `29590190286`, app job `87917050301` and DB job `87917050251` succeeded. Quality artifact `8410790742` (`sha256:a0e7d16d54ecf5f123a2ad4024241a8ebe6cbde79b2aa500eb9bd541a6bf6b8a`) and database artifact `8410824146` (`sha256:d5b1daeca1a3009315ba3494da6db6c64c3c2c88fb6ccd1aeef12ccf7d8661ed`) preserve the receipts.
- Last fully evidenced functional Preview: `dpl_9n6d8RoVN6yEoxRzEALyG3F3rSHP`, READY on that exact head at [geoai-hwg7wapdd-geoaidev.vercel.app](https://geoai-hwg7wapdd-geoaidev.vercel.app). Mock Auth, responsive entry, Workspace analysis-to-print, browser-local project create/reload/open and criteria-first comparison/export/print are browser-proven; all nine scanned surfaces have zero serious/critical Axe findings. Visual regression, Lighthouse/Core Web Vitals, remaining responsive/navigation/target-size acceptance and real-user personas remain unclaimed.
