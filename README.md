...
The replay contract in `supabase/config.toml` exposes only `api` on Postgres 17 and pins Supabase CLI `2.109.1`. On functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`, GitHub run `29496255541`, database job `87613914543`, completed clean start, deterministic reset and the current 71-assertion pgTAP plan with `Files=1, Tests=71`, `All tests successful`, `Result: PASS`. Its 14 red-team additions cover the exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and update/delete immutability for artifacts, status events and ingestion receipts; the policy sweep covers all source custody/audit tables for `public` or authenticated-applicable policies. The overall run is partial, not successful: application job `87613914544` stopped at the stale documentation current-truth gate before this documentation increment was published. A new all-green exact-head Quality Gate and evidence artifact are required after publication. The permanent gate also runs the static source-custody checker, and the guarded operator migration check enumerates all three pending migrations plus canonical/security/identity/source-custody checks. This DB receipt closes only ephemeral clean replay: local Docker remains unavailable, and upgrade replay, live apply, Data API containment, live JWT/Storage/source personas and advisor parity remain open.
...
- Current functional candidate: content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; run `29496255541` is partial/overall failure because app job `87613914544` stopped at the pre-publication documentation current-truth gate, while clean-replay/pgTAP job `87613914543` passed all 71 assertions. A new all-green exact-head Quality Gate and artifact are still required.
- Current audit Preview: `dpl_EaXsfjW5y2WUkCoXoeTYmGiD1DiY`, READY on the exact functional content SHA at [geoai-ltagjva86-geoaidev.vercel.app](https://geoai-ltagjva86-geoaidev.vercel.app). Browser/mobile/keyboard/print evidence remains unclaimed; local headless launch is blocked by a system Unix-socket restriction, and HTTP checks are not browser evidence.
