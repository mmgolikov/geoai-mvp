...
Documentation/evidence snapshot head `f39dcee18c2601b88ae51e061627925037d1aa77` preserves functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`. GitHub Quality Gate run `29497314994` succeeded: application job `87617248273` passed, and Supabase job `87617248275` completed clean start, deterministic reset and the 71-assertion pgTAP plan with `Files=1, Tests=71`, `All tests successful`, `Result: PASS`. Evidence artifact `8374954241` is `geoai-quality-evidence-29497314994`, digest `sha256:e3a5bbfdfb3278e7ed69cb8ab4ba2c8b6137fbbc091064ba38f6b05a5362c73a`. The 14 red-team additions cover the exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and update/delete immutability for artifacts, status events and ingestion receipts; the policy sweep covers all source custody/audit tables for `public` or authenticated-applicable policies. The replay contract in `supabase/config.toml` exposes only `api` on Postgres 17, pins Supabase CLI `2.109.1`, runs the static source-custody checker and guards all three pending migrations. This closes only exact-head ephemeral clean replay: local Docker remains unavailable, and upgrade replay, live apply, Data API containment, live JWT/Storage/source personas and advisor parity remain open.
...
- Current evidence snapshot: head `f39dcee18c2601b88ae51e061627925037d1aa77` over functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; Quality Gate run `29497314994`, app job `87617248273` and Supabase job `87617248275` succeeded; artifact `8374954241` (`geoai-quality-evidence-29497314994`, digest `sha256:e3a5bbfdfb3278e7ed69cb8ab4ba2c8b6137fbbc091064ba38f6b05a5362c73a`).
- Current audit Preview: `dpl_Gh5btUKs8yzySxvpxGnwXjMkYayK`, READY on the exact documentation/evidence head at [geoai-6oirbt1at-geoaidev.vercel.app](https://geoai-6oirbt1at-geoaidev.vercel.app). Browser/mobile/keyboard/print evidence remains unclaimed; local headless launch is blocked by a system Unix-socket restriction, and HTTP checks are not browser evidence.
