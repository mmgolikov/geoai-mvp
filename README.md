...
GeoAI is a Next.js B2B/B2G/B2B2G spatial decision intelligence MVP for real estate/development, construction monitoring and source-backed site screening. Its primary region is MENA/GCC, beginning with the UAE and then Saudi Arabia, Qatar, Oman, Bahrain and Kuwait; Singapore, Hong Kong and other fast-growing Asian cities are phase two. The current release is a public demo prototype, not a production-ready or pilot-ready product. It uses Mapbox, synthetic/demo geospatial layers, deterministic scoring, bounded source-context contracts, comparison dashboards, and print-friendly report previews.

The current externally verified release is merged PR #113 at `7f323c4227f2409f3fe2d4d68be48a30176f4e2a` on both `main` and `release/production`, deployed as Vercel Production `dpl_4yBHCo1eZ7N6GYQWGAg1EdQGwFTE` (`READY`) at https://geoai-mvp.vercel.app. A rollback deployment for this release tuple is not yet verified. See the [Current Release State](docs/CURRENT_RELEASE_STATE.md), [Product Baseline and Readiness](docs/PRODUCT_BASELINE_AND_READINESS.md), stable [release policy](docs/RELEASE_AUTHORITY_POLICY.json) and [external authority registry](docs/EXTERNAL_AUTHORITY_REGISTRY.json). The evidenced stage is `public_demo_prototype`: browser-local demo behavior is active, while protected persistence, confidential pilot operation and real sources remain blocked. Do not enter confidential, regulated, sensitive or client-protected information.
...
## Released Product System baseline

PR #113 released the Product System v3.2.2 correction and its current Production behavior and visuals are the product baseline to preserve. The older Product System v3.2.1 CR 10.02, authority registry, token manifest, component mapping and QA receipt remain historical provenance; their pre-release branch fields do not describe current runtime authority. They authorize no Figma/Code Connect write, page-body redesign, Auth, Supabase, Storage, source activation or another Production action. Draft PR #143 (`product/gcc-real-estate-decision-platform-v1` at `e92fb5d8e8d83de72ee4c4376d958ce598c00536`) is an excluded non-authority and must not be copied, cherry-picked or recreated. See the [Product Baseline and Readiness](docs/PRODUCT_BASELINE_AND_READINESS.md) and [Superseded Document Register](docs/SUPERSEDED_DOCUMENT_REGISTER.md).

## Historical isolated Auth/database rehearsal evidence

GeoAI_main refreshed read-only management metadata for Free project `geoai-auth-rehearsal` (`bkmfcjzalcvdsdvyxpgi`, `eu-west-1`) as `ACTIVE_HEALTHY` on 2026-08-19 (`observedAt` `2026-08-19T20:07:43Z`; valid through `2026-08-20T20:07:43Z`). That label does not read the database. The 2026-07-16 historical receipt recorded execution of six candidate migrations and the owner Data API path, 18 migration-ledger entries, one pre-existing Auth user without recorded project/tenant/protected-resource authority, `183/183` pgTAP personas, a 14-function RPC-only `api` schema, RLS on 29 GeoAI domain tables and complete domain-FK index coverage. These are point-in-time values, not current hosted truth. Current schema/ledger/rows/advisors/RLS/policies/PostgREST/Storage/source state is `unverified`.

That historical receipt closed only its SQL/rehearsal and API-schema isolation increments. A two-session PostgreSQL regression also recorded `create→accept` and `create→revoke` lock-order transitions without deadlock or residual rows; it was table-level runtime evidence, not authenticated RPC/HTTP E2E. Exact Auth-rehearsal head `8e0039260f4cf201b230288b6b02c48d2955600e`, Quality Gate `29534323096` and Preview `dpl_66rk4tVny9TmPjo7BKona5Xo1p1b` are historical receipts that predate the simplified Auth decision below. CR 09.22 executed no real-user browser persona. Real email/phone/Admin personas, first-owner bootstrap, Storage policies/personas, development apply, real sources and Production remain blocked. See the [machine receipt](docs/SUPABASE_AUTH_REHEARSAL_RECEIPT_2026_07_16.json).

## Current Auth product decision
...
The ready mock account is `demo@geoai.space` with password `111111`. These public credentials create browser-only demo state and sample-data access. They are not a Supabase identity, are never trusted by protected server APIs and cannot authorize Admin, customer data or durable writes. The local migration manifest lists `20260716213214_simplify_auth_remove_mfa_requirement.sql` as pending; it replaces the previous AAL2 check with a permanent non-anonymous identity check while preserving roles, RLS, last-owner protection, optimistic concurrency and audit. Its current hosted application state is `unverified`. The helper does not assert verified email or phone ownership. Phone delivery additionally requires an external SMS provider and is not yet runtime-certified.
...
The landing `View demo` action uses `/login?next=/workspace&intent=demo` and preserves the ready browser-only account. Commercial request actions use the separate public `/request-access` route, which formats a plain-text brief in React memory without transmitting or persisting contact data. The legacy Login request intent redirects to that public route. Shared product navigation shows a circular profile icon; an authenticated session highlights it and changes its target to `/profile`. This is an entry-flow and session UX boundary only, not completion of server-side membership/RBAC.
...
This development target is separate from Production. GeoAI_main observed management metadata as `INACTIVE` in a read-only refresh on 2026-08-19 (`observedAt` `2026-08-19T20:07:43Z`; valid through `2026-08-20T20:07:43Z`); physical containment and database state are `unverified`. The 2026-07-16 11:31 UTC historical snapshot recorded `ACTIVE_HEALTHY` on PostgreSQL `17.6.1.141`, 20 public tables/19 using RLS (`spatial_ref_sys` was the exception), zero Auth users, four buckets, zero `storage.objects` policies, 22 public-table `TRUNCATE` grants per public role, 14 security advisor findings and 71 performance advisor findings. The historical rehearsal receipt recorded the first six candidates plus the owner API operator. Local custody now records one pre-ledger reconciliation, exactly ten immutable development-ledger migrations and seven pending migrations with `liveApplyReady:false`; local pending state does not assert current hosted application state. The repository pins Supabase CLI `2.109.1`. Before Auth, real sources or protected files, the owner must expose only `api` (or disable the Data API) and prove development upgrade replay, direct-`public` denial and live caller-JWT/Storage/source personas. The historical [Supabase Data API Containment Runbook](docs/SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md) authorizes no development change.
...
- [Historical Supabase Data API Containment Runbook — rehearsal receipt; development draft](docs/SUPABASE_DATA_API_CONTAINMENT_RUNBOOK_2026_07_16.md)
- Current externally verified `main` and `release/production`: merged PR #113 at `7f323c4227f2409f3fe2d4d68be48a30176f4e2a`.
- Current externally verified Production: `dpl_4yBHCo1eZ7N6GYQWGAg1EdQGwFTE`, READY at https://geoai-mvp.vercel.app on that exact SHA.
- Rollback deployment for the PR #113 tuple: `unverified`; verify before any release action.
