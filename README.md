...
The next candidate extends that permanent suite from one desktop session journey to five browser tests. It checks landing and login layout at 1440px desktop, 834px tablet and 390px mobile widths, rejects horizontal overflow, verifies the public demo/request targets and enforces a 40px application baseline for the three primary mobile entry controls. A keyboard-only mobile journey tabs through `View demo` → `Use demo credentials` → `Open demo` → highlighted profile → `/profile`. Static wiring, Playwright discovery and TypeScript validation pass locally; actual Chrome execution remains pending CI and is not yet claimed. Axe/Lighthouse, deep Workspace analysis/report, project switching and print remain open.
