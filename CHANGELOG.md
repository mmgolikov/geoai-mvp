...
- Staged AUTH-01B's request-scoped, cookie-only read facade. It requires an exact project key, rejects bearer/mixed transport and public-demo fallback, resolves `api.current_project_access()` through the caller client, applies the shared role/action kernel, and maps approved source releases through a strict DTO. Every repository/Auth/persona readiness flag remains false; there is no route activation, base-table access, public cache or service-role path.
- Added SOURCE-02 as a pure provider-neutral connector planning and receipt contract with an empty shipped registry. It has no provider fetch, environment/secret read, credential injection or persistence and denies Production; future local/Preview planning requires objective connector/rights, canonical replay, custody/persona, authenticated-worker, owner, exact-SHA, distributed-rate, cross-instance-circuit and credential-broker evidence plus distribution/geometry/imagery gates where applicable.
...
- Added the static source-custody migration checker to the permanent Quality Gate. Functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289` requires a post-documentation all-green application rerun; run `29496255541` is partial because app job `87613914544` stopped at the stale pre-publication documentation current-truth gate.
...
- Added an `api`-only Postgres 17 `supabase/config.toml`, pinned Supabase CLI `2.109.1` and configured CI `database-replay` for clean start/reset plus pgTAP. Functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289` passed DB job `87613914543` in run `29496255541`: clean start, deterministic reset, `Files=1, Tests=71`, `All tests successful`, `Result: PASS`. The overall run is partial/failed because the application job stopped at stale documentation; a new all-green Quality Gate and artifact are required. The DB receipt closes only ephemeral clean replay; local Docker, upgrade replay, live apply/Data API containment, live personas, advisor parity and DB-01 remain open.
...
- Added a Production dependency audit and AOI adversarial contract to the permanent Quality Gate; the AUTH-01B and SOURCE-02 pure-contract checks are also registered. A post-documentation all-green exact-head run remains required before claiming the complete application gate.
- Replaced the old 133–158 KB repeated source responses with `compact_public_v1`, including `/api/external-data/sources`, 48/64 KB response budgets and Vercel output-trace budgets that exclude deep snapshots. Preview `dpl_EaXsfjW5y2WUkCoXoeTYmGiD1DiY` is READY on exact functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; remote UTF-8 sizes are 5,164/4,411/18,284/5,164/8,221/4,292 B across data-sources/readiness/manifest/sources/status/lineage, with contract 1.3, manifest 1.6 and `liveRegistryIncluded:false`. The disabled legacy AnalysisPanel fanout/dead code, six-request Hub bootstrap and always-on current capture buffer remain residual performance work.
- Lazy-loaded Workspace comparison, express-result and report-preview surfaces behind a stable loading state. On the same local production-build basis, Workspace First Load JS decreased from 252 kB to 218 kB (34 kB, approximately 13.5% gzip). Exact Preview HTTP evidence exists, but browser flows and Core Web Vitals remain unclaimed because local headless launch is blocked by the system Unix-socket boundary.
- Recorded that partial run `29496255541` has no current evidence artifact; a post-publication all-green exact-head Quality Gate must produce the next artifact. No Production, live Supabase, Auth, Storage or provider write was performed.
