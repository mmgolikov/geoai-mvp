...
- Simplified the Find workspace around one scroll region and a drawer-local action footer. Repeated permanent disclaimer blocks and the duplicate mode eyebrow are removed; source, current map extent, incompleteness, prohibited claims, ODbL attribution and the mandatory screening caveat remain available through one `Data & methodology` disclosure.
- Added explicit Search/Update behavior: after a successful bounded Find, a changed viewport, market, locale, audience, role, scenario, object group or mapped-level criterion preserves the original result, shortlist and source lineage as visibly stale and changes the CTA to `Update search`. Browser-session restoration retains the last successful result and its original query context; it does not treat changed UI criteria as new evidence. Analysis navigation from a stale result is disabled while its original market differs from the current map, preventing cross-market identity/context requests.
- Added the V6 product-reconciliation matrix. The V5.1 live map, identity, open-context and reversible Create work remain the candidate foundation; useful prior Workspace patterns are adapted, while old mock ranking and independently recomputed dashboard/report values are retired.
- Defined versioned `GeoContextSnapshot V1`, `Scenario Registry V1` and `DecisionRecord V1` contracts and JSON Schemas. They separate observed, calculated, modelled and hypothesis evidence, keep ranking disabled until comparability gates pass and require dashboard/report to render one immutable decision record.
- Added a dependency/gate sequence, integrated backlog and evidence-backed Generative Development Build/Buy/Partner research. The recommendation is a provider-neutral hybrid: GeoAI owns context, evidence, scenarios, evaluation, economics, explainability, decision workflow and API/MCP; specialist geometry, simulation, digital-twin and BIM components are evaluated through an identical UAE bake-off before adoption.
- Integrated application/documentation head `3f2ca5bda547b52c3c80db44b77a4d8ded058ad9` passed exact Quality Gate `33915252452`: point-to-object Chrome `1/1`, Auth/session/responsive Chrome `35/35`, 79/79-page production build, built-app/API/security smoke and local-only Supabase replay/personas. Protected Preview `dpl_J98acMVSCxAUFQ37noZsy8nt9KPR` is Ready. The V6 receipt binds this head from a documentation-only successor and does not self-certify that successor.
- The branch and PR #147 remain open Draft Preview candidates. Founder manual acceptance, full G2 runtime adapters, any V6.1 generator/provider spike, source/vendor/customer validation, merge and Production release remain pending. No hosted Supabase, Auth/RLS/Storage, source, secret/environment, Figma, Confluence, `main` or Production mutation is included. A disclosed malformed evidence query created orphan GitHub deployment metadata `6271790484` without status, URL, Vercel deployment, Production alias or code change; it remains unresolved pending separate authority.
...
- Added a fail-safe focused-analysis recovery path: the strict evidence validator remains unchanged, but the two known nearby-context binding mismatches are replaced with server-rendered, canonically cited copy before a second model call. Unsupported nearby questions still fail closed when no bound context receipt exists.
...
- Verified application head `789b197e4dcd3dbd03fc3d4aa594bdcb23ab1794` is deployed as Ready protected Preview `dpl_EjHRrkghsrPFXCZZc4cnQWbxWXRa`; Quality Gate `33878620781` passed its exact application checks. The subsequent documentation-only pre-V6 branch head `d85ef69624bc79c50af788c165c0760dcab01c8f` passed Quality Gate `33880472031` and deployed as Ready Preview `dpl_BH4us5PwSs7yw98JfbG9C1GnZgbz`; it did not change the V5.1 runtime. Founder manual acceptance remains pending in the V5.1 receipt. The released Production source pack remains disabled/count-only, and `main`, Production, Figma authority, Confluence synchronization and hosted Supabase are unchanged. No official parcel, cadastral, zoning, ownership, valuation, complete-inventory, architectural-design or approved-plan claim is included.
