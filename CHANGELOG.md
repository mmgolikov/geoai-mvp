...
- Added explicit Search/Update behavior: after a successful bounded Find, a changed viewport, market, locale, audience, role, scenario, object group or mapped-level criterion marks the result stale and changes the CTA to `Update search`; stale output is not persisted as the current session result.
