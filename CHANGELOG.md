...
- Added a Production dependency audit and AOI adversarial contract to the permanent Quality Gate; the AUTH-01B and SOURCE-02 pure-contract checks are also registered for CI. Exact-SHA execution remains pending.
...
- Lazy-loaded Workspace comparison, express-result and report-preview surfaces behind a stable loading state. On the same local production-build basis, Workspace First Load JS decreased from 252 kB to 218 kB (34 kB, approximately 13.5% gzip); exact-Preview and Core Web Vitals evidence remain open.
