...
export function deriveDecisionRationale(analysis: ExpressAnalysis) {
  ...
  if (posture === "Proceed to due diligence") {
    return "The site has a strong illustrative local/public-open screening profile and manageable modeled risk, but conclusions still require validation against official sources before underwriting.";
  }

  if (posture === "Proceed with conditions") {
    return "The site shows useful screening potential, but current evidence remains illustrative local/public-open or partially unvalidated, so next steps should be conditional on official planning, market and infrastructure checks.";
  }
