...
create unique index if not exists ux_pilot_workflows_project_key on public.pilot_workflows(project_key) where project_key is not null;
create unique index if not exists ux_validation_checklist_project_title on public.validation_checklist_items(project_key, title) where project_key is not null;
create unique index if not exists ux_pilot_client_inputs_project_title on public.pilot_client_inputs(project_key, title) where project_key is not null;
create unique index if not exists ux_pilot_deliverables_project_title on public.pilot_deliverables(project_key, title) where project_key is not null;
create unique index if not exists ux_data_room_assets_project_name_type on public.data_room_assets(project_key, name, asset_type) where project_key is not null;
