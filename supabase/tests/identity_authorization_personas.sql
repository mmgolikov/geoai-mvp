...
select extensions.is(
  ...
  1,
  'analyst can read bounded source release metadata for own project'
);
select extensions.is(
...
select extensions.ok(
  not exists (
    ...
    where schemaname = 'public'
      and tablename in ('source_registry_snapshots', 'external_data_snapshots', 'audit_events')
      and 'authenticated' = any(roles)
  ),
  'source custody and audit remain closed to authenticated callers'
);
