...
export function PointObjectProjectsPageClient() {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6" data-testid="point-object-projects-page">
      ...
      {cloudActionMessage ? <p className="mt-4 rounded-xl border border-[#d7dee4] bg-[#f4fbfb] px-4 py-3 text-sm text-[#087f8c]" role="status">{cloudActionMessage}</p> : null}
          ...
          <section key={project.projectId} className={`min-w-0 rounded-2xl border bg-white p-5 shadow-soft ${visibleStore?.activeProjectId === project.projectId ? "border-[#69aaa0]" : "border-line"}`} data-testid="saved-project-card">
            ...
            {project.artifacts.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{project.artifacts.map((artifact) => (
              <article key={artifact.artifactId} data-testid="saved-result-card" className="flex min-w-0 flex-col rounded-xl border border-line bg-[#fbfcfd] p-4">
                <div className="flex items-center justify-between gap-2"><span className="rounded-full bg-[#e5fafa] px-2.5 py-1 text-[10px] font-bold uppercase text-[#344054]">{artifactKindLabel(artifact.kind, locale)}</span><time className="text-[10px] text-muted">{new Date(artifact.completedAt).toLocaleDateString(locale)}</time></div>
                ...
                <button type="button" onClick={() => void reopen(project.projectId, artifact)} disabled={openingArtifactId === artifact.artifactId} title={locale === "ru" ? "Без повторного запроса к источнику" : "Without rerunning a source request"} className="mt-4 min-h-11 rounded-xl bg-[#087f8c] px-3 text-xs font-bold text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] focus-visible:ring-offset-2 disabled:cursor-wait disabled:bg-[#d7dee4]">{openingArtifactId === artifact.artifactId ? (locale === "ru" ? "Открываем…" : "Opening…") : artifact.kind === "analyse" ? (locale === "ru" ? "Открыть результат" : "Open result") : (locale === "ru" ? "Показать на карте" : "Show on map")}</button>
              </article>
