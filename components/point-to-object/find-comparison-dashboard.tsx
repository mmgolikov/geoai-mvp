    ...
    <section ref={dialogRef} className="fixed inset-0 z-[70] overflow-y-auto bg-[#f4f7f6] text-ink" role="dialog" aria-modal="true" aria-labelledby="find-comparison-dashboard-title" data-testid="find-full-comparison-dashboard">
      <header className="sticky top-0 z-10 border-b border-line bg-white/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3">
          <button data-modal-initial-focus type="button" onClick={onBackToComparison} className="min-h-11 rounded-xl border border-[#9fc7bd] bg-white px-4 text-sm font-bold text-[#087f8c] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c]">← {ru ? "К краткому сравнению" : "Back to compact comparison"}</button>
      ...
      <div className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6 lg:p-8">
        <section className="rounded-[24px] border border-[#cfe0dc] bg-white p-5 shadow-soft sm:p-7">
          ...
          <p className="mt-3 inline-flex rounded-full bg-[#e6f5f1] px-3 py-1.5 text-xs font-bold text-[#087f8c]" data-testid="find-comparison-basis">{ru ? "Основа: наблюдаемые атрибуты текущей выборки OSM · без отдельного AI-сравнения" : "Basis: observed attributes in this OSM sample · no separate comparison AI run"}</p>
