...
export function PointToObjectPrototypeV5({ initialMode = "analyse" }: { initialMode?: ProductMode } = {}) {
  ...
  useEffect(() => {
    ...
    const applyResolved = (resolvedObject: NonNullable<ReturnType<typeof parseLiveResolvedObject>>) => {
      ...
      setSelection((current) => current && contextRequestKey(current, locale) === unresolvedContextKey ? {
        ...
        object: mergePointObjectContextGeometry(current.object, resolvedObject),
        resolvedObject
      } : current);
      setContextStatus("idle");
    };
  ...
  function chooseFindCandidate(candidate: PointObjectFindResult["candidates"][number]) {
    ...
    setNavigationTarget({
      ...
      exactFindCandidate: candidate,
      resolvedFindContext: findResolvedObjects[candidate.sourceFeatureId] ?? null
    });
  }
    ...
    <main ref={workspaceRef} data-project-restoration={projectRestorationReady ? "ready" : "loading"} className={`${mobileStyles.workspace} overflow-hidden bg-white text-ink`}>
      ...
      <div className={mobileStyles.shell} data-sheet={effectiveSheet} data-testid="mobile-workspace-shell">
          ...
          <div className="absolute left-3 top-3 z-10 flex w-[min(650px,calc(100%-4.5rem))] flex-row gap-2 sm:left-5 sm:top-5">
            ...
            <form onSubmit={searchPlace} role="search" className="relative min-w-0 flex-1">
              <div className="flex h-11 overflow-hidden rounded-xl border border-white/70 bg-white/95 shadow-[0_10px_30px_rgba(20,35,45,0.14)] backdrop-blur focus-within:ring-2 focus-within:ring-[#087f8c]">
                ...
                <button type="submit" disabled={searchQuery.trim().length < 2 || searchStatus === "loading"} className="min-w-[76px] bg-[#087f8c] px-3 text-xs font-bold text-white transition hover:bg-[#087f8c] disabled:cursor-not-allowed disabled:bg-[#d7dee4] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white">{searchStatus === "loading" ? t("search.loading") : t("search.action")}</button>
              </div>
              {searchResults.length ? <div id="point-object-search-results" role="listbox" className="absolute left-0 right-0 top-[calc(100%+0.5rem)] max-h-72 overflow-y-auto rounded-xl border border-line bg-white p-1.5 shadow-panel" aria-label={t("search.results")}>
                {searchResults.map((result, index) => <button id={`point-object-search-result-${index}`} key={result.id} type="button" role="option" aria-selected={activeSuggestionIndex === index} onMouseEnter={() => setActiveSuggestionIndex(index)} onClick={() => chooseSearchResult(result)} className={`block min-h-11 w-full rounded-lg px-3 py-2 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] ${activeSuggestionIndex === index ? "bg-[#e5fafa]" : "hover:bg-[#f4fbfb]"}`}><span className="block text-sm font-bold text-ink">{result.label}</span>{result.secondaryLabel ? <span className="mt-0.5 block truncate text-[11px] text-muted">{result.secondaryLabel}</span> : null}</button>)}
        ...
        {mode === "create" && projectRestorationReady && isDrawing ? <div className={mobileStyles.drawTools} data-testid="create-map-drawing-tools" data-editing={Boolean(createAoi)}>
          ...
          <div className="grid grid-cols-3 gap-2"><button disabled={draftCoordinates.length < 3} onClick={() => closeCreateArea()} className="rounded-lg bg-[#087f8c] text-xs font-bold text-white disabled:opacity-40">{locale === "ru" ? "Завершить зону" : "Finish area"}</button><button disabled={!draftCoordinates.length} onClick={() => setDraftCoordinates((current) => current.slice(0, -1))}>{t("create.undo")}</button><button onClick={cancelCreateDrawing}>{t("create.cancel")}</button></div>
        ...
        <aside id="workspace-task" className={`${mobileStyles.sheet} min-h-0 min-w-0 overflow-hidden border-l border-line bg-white`} aria-label={locale === "ru" ? "Задача" : "Task"} onKeyDown={(event) => { if (event.key === "Escape" && !event.defaultPrevented && sheet === "full") { event.stopPropagation(); showMap(); } }}>
          <div className={mobileStyles.drawer}>
            ...
            {mode === "find" ? <section className="mt-2 flex min-h-0 flex-1 flex-col" data-testid="find-drawer">
              ...
              {findResult ? <div className="mt-4">
                ...
                {findComparisonOpen && findShortlist.length >= 2 ? <div className="-mx-1 overflow-x-auto overflow-y-hidden px-1 pb-2" role="region" aria-label={locale === "ru" ? "Сравнение объектов" : "Object comparison"} tabIndex={0}>
                  ...
                  const subtype = readableFindSubtype(candidate.matchedTag.value, candidate.group, locale);
                  return <li key={candidate.sourceFeatureId} onMouseEnter={() => setHoveredFindResultId(candidate.sourceFeatureId)} onMouseLeave={() => setHoveredFindResultId(null)} className={`rounded-xl border bg-white p-3 ${activeFindResultId === candidate.sourceFeatureId ? "border-[#087f8c] ring-2 ring-[#e5fafa]" : selectedForComparison ? "border-[#d7dee4]" : "border-line"}`}>
                    <button id={`find-result-${candidate.sourceFeatureId}`} type="button" disabled={findResultIsStale} onFocus={() => setHoveredFindResultId(candidate.sourceFeatureId)} onBlur={() => setHoveredFindResultId(null)} onClick={() => focusFindResult(candidate.sourceFeatureId)} className="min-h-11 w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] disabled:cursor-not-allowed disabled:opacity-40"><span className="mr-2 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[#e5fafa] px-1 text-[10px] font-bold text-[#344054]">{findResult.candidates.indexOf(candidate) + 1}</span><span className="text-sm font-bold text-ink">{candidate.label}</span><span className="mt-1 block text-[11px] text-muted">{findGroupLabels[candidate.group]}{subtype ? ` · ${subtype}` : ""}{candidate.mappedBuildingLevels === null ? "" : ` · ${candidate.mappedBuildingLevels} ${locale === "ru" ? "эт." : "levels"}`}</span></button>
                    <div className="mt-2 flex flex-wrap gap-2"><button type="button" disabled={findResultIsStale} onClick={() => chooseFindCandidate(candidate)} className="min-h-11 rounded-lg border border-[#d7dee4] bg-white px-3 text-[11px] font-bold text-[#087f8c] disabled:opacity-40">{locale === "ru" ? "Открыть анализ" : "Open analysis"}</button><button type="button" aria-pressed={selectedForComparison} disabled={findResultIsStale || (!selectedForComparison && findShortlist.length >= 3)} onClick={() => toggleFindShortlist(candidate)} className={`min-h-11 rounded-lg px-3 text-[11px] font-bold transition disabled:opacity-40 ${selectedForComparison ? "bg-[#087f8c] text-white" : "border border-[#d7dee4] bg-white text-[#087f8c]"}`}>{selectedForComparison ? (locale === "ru" ? "Выбрано" : "Selected") : (locale === "ru" ? "В сравнение" : "Compare")}</button></div>
                  </li>;
                })}</ul>}
              </div> : null}
              </div>
              <footer className="sticky bottom-0 z-20 shrink-0 border-t border-line bg-white pt-1" data-testid="find-sticky-footer">
                ...
                <button
                  ...
                  className="min-h-11 w-full rounded-xl bg-[#087f8c] px-4 text-sm font-bold text-white transition hover:bg-[#087f8c] disabled:cursor-not-allowed disabled:bg-[#b7c4c4] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] focus-visible:ring-offset-2"
            ...
            <div hidden={mode !== "create"} inert={!projectRestorationReady} aria-busy={!projectRestorationReady || undefined} data-testid="create-workspace" className="mt-5 space-y-4">
              {!createAoi ? <section className="rounded-[18px] border border-[#d7dee4] bg-[#f4fbfb] p-4">
                {canRestoreRemovedCreate ? <button type="button" data-testid="create-undo-remove" onClick={() => { restoreRemovedCreateRef.current?.(); restoreRemovedCreateRef.current = null; setCanRestoreRemovedCreate(false); }} className="mb-3 min-h-11 w-full rounded-xl border border-[#d7dee4] bg-white px-3 text-xs font-bold text-[#344054]">{locale === "ru" ? "Вернуть удалённую зону" : "Undo area removal"}</button> : null}
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => { setIsDrawing(true); setSheet("peek"); setCreateError(null); }} className="min-h-11 rounded-xl bg-[#087f8c] px-3 text-xs font-bold text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] focus-visible:ring-offset-2">{draftCoordinates.length ? (locale === "ru" ? "Редактировать контур" : "Edit drawing") : t("create.draw")}</button>
                  <label className="grid min-h-11 cursor-pointer place-items-center rounded-xl border border-[#d7dee4] bg-white px-3 text-center text-xs font-bold text-[#344054] focus-within:ring-2 focus-within:ring-[#087f8c]"><span>{t("create.upload")}</span><input type="file" accept="application/geo+json,application/json,.geojson,.json" aria-label={t("create.upload")} onChange={(event) => void uploadCreateArea(event)} className="sr-only focus-visible:outline-none" /></label>
                </div>
                <button type="button" data-testid="create-use-selection" disabled={!selectedCreateVertices} onClick={() => { if (selectedCreateVertices) closeCreateArea(selectedCreateVertices, true); }} className="mt-2 min-h-11 w-full rounded-xl border border-[#d7dee4] bg-white px-3 text-xs font-bold text-[#344054] disabled:opacity-50">{locale === "ru" ? "Использовать выбранный объект" : "Use selected object"}</button>
                ...
                {isDrawing ? <><p className="mt-3 text-xs font-semibold text-[#344054]">{t("create.drawing", { count: draftCoordinates.length })}</p><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" disabled={draftCoordinates.length < 3} onClick={() => closeCreateArea()} className="min-h-11 rounded-lg bg-[#087f8c] px-2 text-xs font-bold text-white disabled:opacity-40">{locale === "ru" ? "Завершить зону" : "Finish area"}</button><button type="button" disabled={!draftCoordinates.length} onClick={() => setDraftCoordinates((current) => current.slice(0, -1))} className="min-h-11 rounded-lg border border-[#d7dee4] bg-white px-2 text-xs font-bold text-[#344054] disabled:opacity-40">{t("create.undo")}</button><button type="button" onClick={cancelCreateDrawing} className="min-h-11 rounded-lg border border-[#d7dee4] bg-white px-2 text-xs font-bold text-[#344054]">{t("create.cancel")}</button></div></> : null}
              ...
              </section> : <><p className="rounded-xl border border-[#d7dee4] bg-[#f4fbfb] px-4 py-3 text-xs font-bold text-[#344054]">{t("create.ready", { area: createAoi.areaSqM >= 10_000 ? `${(createAoi.areaSqM / 10_000).toFixed(2)} ${locale === "ru" ? "га" : "ha"}` : `${Math.round(createAoi.areaSqM).toLocaleString(locale)} ${locale === "ru" ? "м²" : "m²"}` })}</p>
                <section className="rounded-[18px] border border-line bg-[#f8fafc] p-4" aria-live="polite">
                  <div className="flex flex-col items-start gap-3">
                    <div className="w-full min-w-0">
                      <p className="text-[11px] font-bold uppercase tracking-[0.09em] text-[#087f8c]">{locale === "ru" ? "КОНТЕКСТ ЗОНЫ" : "AREA CONTEXT"}</p>
                    ...
                    <button type="button" data-testid="create-map-presentation-toggle" onClick={toggleCreateMapPresentation} className="min-h-11 max-w-full rounded-lg border border-[#d7dee4] bg-white px-3 text-left text-[11px] font-bold text-[#087f8c] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c]">{createAreaCleared ? (locale === "ru" ? "Показать исходные" : "Show existing") : activeConceptMassing ? (locale === "ru" ? "Показать созданную концепцию" : "Show generated concept") : (locale === "ru" ? "Скрыть исходные здания" : "Hide existing buildings")}</button>
                  </div>
                  {areaContextStatus === "loading" ? <p className="mt-3 text-xs font-semibold text-[#087f8c]" role="status">{locale === "ru" ? "Собираем объекты открытой карты…" : "Reading open-map objects…"}</p> : null}
                ...
                <button type="button" data-testid="create-edit-area" onClick={() => { setDraftCoordinates(createAoi.coordinates[0].slice(0, -1)); setIsDrawing(true); setSheet("peek"); }} className="min-h-11 w-full rounded-xl border border-[#d7dee4] bg-white px-3 text-xs font-bold text-[#344054]">{locale === "ru" ? "Изменить границу" : "Edit boundary"}</button><button type="button" data-testid="create-delete-area" onClick={resetCreate} className="min-h-11 w-full rounded-xl border border-[#d7dee4] bg-white px-3 text-xs font-bold text-[#344054] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c]">{t("create.deleteArea")}</button></>}
            </div>
            </div>
          </div>
        </aside>
