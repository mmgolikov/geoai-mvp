              ...
              {findResult ? <div className="mt-4">
                ...
                {findShortlist.length > 0 ? <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-[#e6f5f1] px-3 py-2" data-testid="find-comparison-toolbar">
                  ...
                  <div className="flex flex-wrap justify-end gap-1.5">
                    {findComparisonOpen ? <button type="button" disabled={findResultIsStale} onClick={() => setFindComparison(false)} className="min-h-11 rounded-lg border border-[#8ebdb4] bg-white px-3 text-[11px] font-bold text-[#087f8c] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] disabled:cursor-not-allowed disabled:opacity-40">{locale === "ru" ? "К результатам" : "Back to results"}</button> : null}
                ...
                {findComparisonOpen && findShortlist.length >= 2 ? <div className="-mx-1 overflow-x-auto overflow-y-hidden px-1 pb-2" role="region" aria-label={locale === "ru" ? "Сравнение объектов" : "Object comparison"} tabIndex={0}>
                  ...
                  return <li key={candidate.sourceFeatureId} onMouseEnter={() => setHoveredFindResultId(candidate.sourceFeatureId)} onMouseLeave={() => setHoveredFindResultId(null)} className={`rounded-xl border bg-white p-3 ${activeFindResultId === candidate.sourceFeatureId ? "border-[#087f8c] ring-2 ring-[#bfe4e2]" : selectedForComparison ? "border-[#8ebdb4]" : "border-line"}`}>
                    <button id={`find-result-${candidate.sourceFeatureId}`} type="button" disabled={findResultIsStale} onFocus={() => setHoveredFindResultId(candidate.sourceFeatureId)} onBlur={() => setHoveredFindResultId(null)} onClick={() => focusFindResult(candidate.sourceFeatureId)} className="min-h-11 w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c] disabled:cursor-not-allowed disabled:opacity-40"><span className="mr-2 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[#e6f5f1] px-1 text-[10px] font-bold text-[#087f8c]">{findResult.candidates.indexOf(candidate) + 1}</span><span className="text-sm font-bold text-ink">{candidate.label}</span><span className="mt-1 block text-[11px] text-muted">{findGroupLabels[candidate.group]}{subtype ? ` · ${subtype}` : ""}{candidate.mappedBuildingLevels === null ? "" : ` · ${candidate.mappedBuildingLevels} ${locale === "ru" ? "эт." : "levels"}`}</span></button>
                    <div className="mt-2 flex flex-wrap gap-2"><button type="button" disabled={findResultIsStale} onClick={() => chooseFindCandidate(candidate)} className="min-h-11 rounded-lg border border-[#8ebdb4] bg-white px-3 text-[11px] font-bold text-[#087f8c] disabled:opacity-40">{locale === "ru" ? "Открыть анализ" : "Open analysis"}</button><button type="button" aria-pressed={selectedForComparison} disabled={findResultIsStale || (!selectedForComparison && findShortlist.length >= 3)} onClick={() => toggleFindShortlist(candidate)} className={`min-h-11 rounded-lg px-3 text-[11px] font-bold transition disabled:opacity-40 ${selectedForComparison ? "bg-[#087f8c] text-white" : "border border-[#8ebdb4] bg-white text-[#087f8c]"}`}>{selectedForComparison ? (locale === "ru" ? "Выбрано" : "Selected") : (locale === "ru" ? "В сравнение" : "Compare")}</button></div>
                  </li>;
                })}</ul>}
              </div> : null}
              </div>
              ...
              {!createAoi ? <section className="rounded-[18px] border border-[#cfe0da] bg-[#f4faf7] p-4">
                ...
                <section className="rounded-[18px] border border-line bg-[#f8fafc] p-4" aria-live="polite">
                  <div className="flex flex-col items-start gap-3">
                    ...
                    <button type="button" data-testid="create-map-presentation-toggle" onClick={toggleCreateMapPresentation} className="min-h-11 max-w-full rounded-lg border border-[#8ebdb4] bg-white px-3 text-left text-[11px] font-bold text-[#087f8c] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#087f8c]">{createAreaCleared ? (locale === "ru" ? "Показать исходные" : "Show existing") : activeConceptMassing ? (locale === "ru" ? "Показать созданную концепцию" : "Show generated concept") : (locale === "ru" ? "Скрыть исходные здания" : "Hide existing buildings")}</button>
                  </div>
