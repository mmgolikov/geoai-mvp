...
import { dashboardCategoryRows, dashboardHeroMetrics, dashboardLayout, decisionContextSummary, decisionViews, POINT_OBJECT_DASHBOARD_VERSION, type DecisionCardId, type DashboardModule } from "@/src/lib/prototype/point-to-object-dashboard-registry";
...
export function PointObjectDecisionCards({ context, generatedAt, reportPerspective, places, groupLabels, districtLabels, request, content }: Props) {
  ...
  const [evidenceFilter, setEvidenceFilter] = useState<"all" | "observed" | "derived" | "hypothesis">("all");
  const layout = dashboardLayout(request);
  const rows = dashboardCategoryRows(context, request.scenario);
  const heroMetrics = dashboardHeroMetrics(request, context);
  const selected = rows.find(row => row.group === category);
  const questionAnswer = content.answerToQuestion;
  ...
  const ru = locale === "ru";
  const evidenceLabels = { all: ru ? "Все" : "All", observed: ru ? "Из источника" : "Source observations", derived: ru ? "Расчёты" : "Derived", hypothesis: ru ? "Гипотезы" : "Hypotheses" };
  const checks = review?.analyticChecks ?? content.signals;
  const visibleChecks = evidenceFilter === "all" ? checks : checks.filter(item => item.evidenceClass === evidenceFilter);
  const validationSteps = [...content.nextValidation].sort((left, right) => ["critical", "high", "medium"].indexOf(left.priority) - ["critical", "high", "medium"].indexOf(right.priority));
  ...
  function body(id: DecisionCardId) {
    switch (id) {
      ...
      case "building_context": return <>
        ...
        {data.available && context && Number.isInteger(context.mappedLevelsKnownCount) && context.mappedLevelsKnownCount >= 0 && context.mappedBuildingCount > 0 && context.mappedLevelsKnownCount <= context.mappedBuildingCount ? <div className={styles.completeness}>
          <meter min={0} max={context.mappedBuildingCount} value={context.mappedLevelsKnownCount} className={styles.meter} aria-label={ru ? "Доля зданий с этажностью в выборке" : "Share of sampled buildings with level data"} />
          <div><span>{ru ? "Этажность известна" : "Levels recorded"}: {context.mappedLevelsKnownCount}</span><span>{ru ? "Неизвестна" : "Unknown"}: {context.mappedBuildingCount - context.mappedLevelsKnownCount}</span></div>
        </div> : null}
      </>;
  ...
  function moduleBody(id: DashboardModule) {
    ...
    if (id === "coverage") return <>
      ...
      <details data-testid="dashboard-evidence-inspection" open={layout.lead === "coverage"}><summary>{ru ? "Проверки по типу основания" : "Inspect evidence classes"}</summary>
      <div className={styles.evidenceFilter} role="group" aria-label={ru ? "Проверки по типу основания" : "Checks by evidence class"}>{(["all", "observed", "derived", "hypothesis"] as const).map(value => <button type="button" key={value} className={styles.localButton} aria-pressed={evidenceFilter === value} aria-controls="dashboard-evidence-checks" data-evidence-filter={value} onClick={() => setEvidenceFilter(value)}>{evidenceLabels[value]} <span>({value === "all" ? checks.length : checks.filter(item => item.evidenceClass === value).length})</span></button>)}</div>
      <div id="dashboard-evidence-checks" aria-live="polite" className={styles.checks}>{visibleChecks.length ? visibleChecks.map((item, index) => <details key={`${item.evidenceClass}:${item.title}:${index}`} data-evidence-class={item.evidenceClass}><summary><span>{item.title}</span><small>{evidenceLabels[item.evidenceClass]}</small></summary><p>{item.observation}</p><p className="text-muted">{item.implication}</p>{refs(item.evidenceRefs)}</details>) : <p className="text-sm text-muted">{ru ? "Проверок этого типа в сохранённом результате нет." : "No checks of this class in the saved result."}</p>}</div>
      </details>
    </>;
    if (id === "risks") return <ul className={styles.items}>{content.risks.map((risk,i) => <li key={i}><details><summary>{risk.title}<small className={styles.itemLabel}>{risk.severity === "high" ? (ru ? "Высокая значимость" : "High significance") : risk.severity === "medium" ? (ru ? "Средняя значимость" : "Medium significance") : (ru ? "Низкая значимость" : "Low significance")}</small></summary><p>{risk.statement}</p><p className="text-muted">{risk.decisionImpact}</p>{refs(risk.evidenceRefs)}</details></li>)}</ul>;
    if (id === "validation") return <ol className={styles.steps}>{validationSteps.map((item,i) => <li key={i}><span className={styles.stepNumber} aria-hidden="true">{i+1}</span><div><h4>{item.title}<small className={styles.itemLabel}>{item.priority === "critical" ? (ru ? "Критический шаг" : "Critical step") : item.priority === "high" ? (ru ? "Высокий приоритет" : "High priority") : (ru ? "Средний приоритет" : "Medium priority")}</small></h4><p>{item.action}</p><details><summary>{ru ? "Где проверить и что изменится" : "Where to verify & decision impact"}</summary><p>{item.source} · {item.decisionImpact}</p>{refs(item.evidenceRefs)}</details></div></li>)}</ol>;
  ...
  return <section className={styles.dashboard} data-testid="role-decision-cards" data-goal={request.goal} data-depth={request.depth} data-scenario={request.scenario} data-dashboard-version={POINT_OBJECT_DASHBOARD_VERSION} aria-labelledby="decision-dashboard-title">
    ...
    <dl className={styles.kpis} data-testid="dashboard-hero-metrics">{heroMetrics.map(metric => <div key={metric.id} data-metric={metric.id}><dt>{metric.label[locale]}</dt><dd>{metric.value === null ? "—" : typeof metric.value === "number" ? metric.value.toLocaleString(locale) : metric.value}{metric.value !== null && metric.unit ? ` ${ru ? "м" : "m"}` : ""}</dd></div>)}</dl>
    {validationSteps[0] ? <aside className={styles.next}><strong>{ru ? "Следующее действие" : "Next action"}</strong><p>{validationSteps[0].action}</p><small>{validationSteps[0].source}</small></aside> : null}
    <div className={styles.grid}>{layout.modules.map(id => <article key={id} data-module={id} className={`${styles.module} ${layout.wideModules.includes(id) ? styles.wide : ""}`}><h3 className="text-lg font-bold">{moduleTitles[id]}</h3>{moduleBody(id)}</article>)}</div>
