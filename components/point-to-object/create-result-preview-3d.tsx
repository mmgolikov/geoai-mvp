...
export function CreateResultPreview3D({ locale, aoi, massing, fallback }: Props) {
  ...
  const appliedGeometryKeyRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    const cleanupRuntime = () => {
      ...
      resizeObserver = null;
      if (map) {
        if (failHandler) map.off("error", failHandler);
        map.remove();
        if (mapRef.current === map) mapRef.current = null;
        map = null;
      }
      appliedGeometryKeyRef.current = null;
    };
    ...
    void import("maplibre-gl").then((maplibregl) => {
      ...
      try {
        ...
        map.on("error", failHandler);
        map.once("load", () => {
          if (disposed || !modelRef.current || !map) return;
          try {
  ...
  if (!model || status === "unsupported" || status === "error") {
    const fallbackStatus = model ? status : "invalid";
    return <div className="space-y-3" data-testid="create-result-preview-3d-fallback" data-preview-status={fallbackStatus}>
