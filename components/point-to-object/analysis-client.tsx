...
function dispositionStyle(value: "continue_screening" | "hold" | "insufficient_evidence"): string {
  if (value === "continue_screening") return "border-[#e5fafa] bg-[#f4fbfb] text-[#087f8c]";
...
function evidenceClassStyle(value: "observed" | "derived" | "hypothesis"): string {
  if (value === "observed") return "bg-[#f4fbfb] text-[#087f8c]";
...
function DepthReviewPanel({ review }: { review: PointObjectDepthReview }) {
  ...
  return <section className="rounded-[20px] border border-[#d7dee4] bg-white p-5 shadow-soft sm:p-7" data-testid="analysis-depth-review" data-depth={review.depth}>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.1em] text-[#087f8c]">{ru ? "ГЛУБИНА АНАЛИЗА" : "ANALYSIS DEPTH"}</p><h2 className="mt-2 text-xl font-bold tracking-[-0.02em]">{title}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-muted">{intro}</p></div><span className="rounded-full bg-[#e5fafa] px-3 py-1 text-xs font-bold text-[#344054]">{review.depth === "quick" ? t("analysis.quick") : review.depth === "standard" ? t("analysis.standard") : t("analysis.deep")}</span></div>
              ...
              <div className="space-y-5" data-testid="ai-success">
                ...
                {geoContext ? <details className="rounded-[20px] border border-line bg-white p-5 shadow-soft sm:p-7" data-testid="analysis-geocontext">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${geoContext.coverage === "available" ? "bg-[#f4fbfb] text-[#087f8c]" : "bg-[#fff5e8] text-[#8a4b08]"}`}>{geoContext.coverage === "available" ? (locale === "ru" ? "Данные доступны" : "Coverage available") : (locale === "ru" ? "Данные недоступны" : "Coverage unavailable")}</span>
                  </div>
                  {geoContext.coverage === "available" ? <>
                    <div className="mt-4 rounded-2xl bg-[#f4fbfb] p-4"><p className="text-[11px] font-bold uppercase tracking-[0.06em] text-[#087f8c]">{locale === "ru" ? "Характер окружения" : "District character"}</p><p className="mt-1 text-lg font-bold text-[#344054]">{districtLabels[geoContext.districtCharacter.code] ?? humanizeAttribute(geoContext.districtCharacter.code)}</p><p className="mt-1 text-xs leading-5 text-[#667085]">{locale === "ru" ? "Определяющие группы:" : "Primary drivers:"} {geoContext.districtCharacter.driverGroups.map((group) => contextGroupLabels[group] ?? group).join(", ") || (locale === "ru" ? "сигнал недостаточен" : "insufficient signal")} · {confidenceLabel(geoContext.districtCharacter.confidence)}</p></div>
                  ...
                  <section className="rounded-[20px] border border-line bg-white p-5 shadow-soft">
                    ...
                    <ul className="mt-4 space-y-3">
                      {content.opportunities.map((item, index) => (
                        <li key={`${item.title}-${index}`} className="rounded-2xl bg-[#f4fbfb] p-4">
                          <h3 className="text-sm font-bold text-[#087f8c]">{item.title}</h3><p className="mt-2 text-sm leading-6 text-[#344054]">{item.hypothesis}</p><p className="mt-2 text-xs leading-5 text-[#475467]">{item.rationale}</p><p className="mt-2 text-xs font-semibold leading-5 text-[#087f8c]">{t("analysis.potentialValue")} {item.potentialValue}</p><p className="mt-2 text-[11px] leading-4 text-muted">{t("analysis.evidenceNeeded")} {item.evidenceNeeded.join("; ")}</p><EvidenceRefs references={item.evidenceRefs} />
                        </li>
                      ))}
                    </ul>
                  </section>
                  ...
                  <ol className="mt-4 space-y-3">
                    {content.nextValidation.map((item, index) => (
                      <li key={`${item.title}-${index}`} className="grid gap-3 rounded-xl border border-line p-4 sm:grid-cols-[32px_minmax(0,1fr)]">
                        <span className="grid h-8 w-8 place-items-center rounded-full bg-[#e5fafa] text-xs font-bold text-[#344054]">{index + 1}</span>
        ...
        <aside className="min-w-0"><div className="space-y-5 xl:sticky xl:top-6">
          <form onSubmit={submitFollowUp} className="rounded-[20px] border border-line bg-white p-5 shadow-soft">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2" aria-label={t("analysis.focusedOptions")}>
              {focusedAnalyses.map((item) => <button key={item.goal} type="button" onClick={() => selectFocusedAnalysis(item.goal, item.question)} disabled={loading} aria-pressed={goal === item.goal} className={`min-h-11 rounded-xl border px-3 text-left text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${goal === item.goal ? "border-[#087f8c] bg-[#e5fafa] text-[#344054]" : "border-line bg-[#f8fafc] text-[#344054] hover:border-[#087f8c] hover:bg-white"}`}>{item.label}</button>)}
            </div>
            <textarea id="analysis-follow-up" value={question} onChange={(event) => { const draft = event.target.value.slice(0, 500); setQuestion(draft); setGoal("custom"); if (selection) setDraftStorageFailed(!writePointObjectQuestionDraft(draft, questionScope(selection))); }} rows={5} placeholder={t("analysis.followUpPlaceholder")} className="mt-4 w-full resize-y rounded-xl border border-line p-3 text-sm leading-6 outline-none focus:border-[#087f8c] focus:ring-2 focus:ring-[#e5fafa]" />
