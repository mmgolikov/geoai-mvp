...
export function PointObjectProjectControl() {
  ...
  return (
    <div className="flex min-w-0 items-center gap-1.5" data-testid="point-object-project-control">
      ...
      {identityKey && store ? <span data-testid="project-storage-location" role="img" aria-label={locale === "ru" ? "На этом устройстве — облачная синхронизация не включена" : "On this device — cloud sync is not enabled"} title={locale === "ru" ? "Проекты в этом браузере на этом устройстве. Облачная синхронизация не включена." : "Projects in this browser on this device. Cloud sync is not enabled."} className="hidden h-11 w-6 shrink-0 items-center justify-center sm:inline-flex"><img src="/brand/storage-device.svg" alt="" width={20} height={20} /></span> : null}
