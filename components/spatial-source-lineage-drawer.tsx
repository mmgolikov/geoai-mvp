...
export function SpatialSourceLineageDrawer({ objectName, lineage, onClose, returnFocusTo }: SpatialSourceLineageDrawerProps) {
  const dialogRef = useAccessibleModal({ open: true, onClose, returnFocusTo });
    ...
    >
      ...
      >
        <div className="sticky top-0 z-10 -mx-1 flex items-start justify-between gap-3 bg-surface px-1 pb-3">
          ...
          <button type="button" onClick={onClose} aria-label="Close source lineage" data-modal-initial-focus className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-line bg-white text-sm font-semibold text-muted transition hover:border-brand hover:text-ink">
            X
          </button>
        </div>
        ...
        <div className="grid gap-2">
          ...
          <LineageSection title="4. Provider IDs and aliases">
            <p>Source ID: <span className="break-all" data-lineage-source-id>{lineage.sourceId}</span></p>
            <p className="mt-1">Provider feature ID: <span className="break-all" data-lineage-provider-feature-id>{lineage.providerFeatureId ?? "Not recorded"}</span></p>
            <p className="mt-1">Source record ID: <span className="break-all">{lineage.sourceRecordId ?? "Not recorded"}</span></p>
            {lineage.sourceAliases.length ? (
              <ul className="mt-1 grid gap-1 text-muted">
                {lineage.sourceAliases.map((alias) => <li key={`${alias.sourceId}:${alias.sourceFeatureId}`} className="break-all">{alias.sourceId}: {alias.sourceFeatureId}</li>)}
              </ul>
          ...
          <LineageSection title="8. Limitations and required validation">
            <ul className="grid gap-1 text-muted">{lineage.limitations.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul>
