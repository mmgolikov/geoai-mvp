    ...
    <article className="geoai-print-report">
      <PrintPage>
        ...
        <div className="geoai-print-meta-grid">
          ...
          <PrintCard label="Demo screening preference" value={report.winnerLabel} />
          <PrintCard label="Generated by" value={report.generatedBy} />
        </div>

        <div className="geoai-print-top-grid">
          <PrintSection title="Schematic Comparison Context">
            <ReportPrintMap
              title="Comparison context"
              subtitle="Schematic selected sites / assets"
              comparison={report}
              geometryLabel="Print-safe screening context; official validation required"
            />
          </PrintSection>

          <PrintSection title="Screening Preference / Trade-offs">
          ...
          <div className="geoai-print-score-grid">
            {report.comparedItems.map((item, index) => (
              <PrintCard
                ...
                note={`Demo deterministic score · ${item.riskLevel} / ${item.recommendedUse}`}
              />
            ))}
          </div>
        </PrintSection>
      ...
      <PrintPage>
        <PrintSection title="Score Comparison">
          {comparison ? (
            <>
              <p className="geoai-print-note">All values below are demo deterministic screening scores, not valuation or underwriting conclusions.</p>
            ...
            <div className="geoai-print-score-grid">
              {report.comparedItems.map((item, index) => (
                <PrintCard
                  key={`fallback-score-${index}`}
                  label={item.name}
                  value={item.overallScore ?? "N/A"}
                  note={`Demo deterministic score · ${item.keyConcern}`}
                />
              ))}
            </div>
          )}
