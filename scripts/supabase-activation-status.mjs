...
async function probeApiHealth() {
  ...
  try {
    ...
    const projectRef = target.protocol === "https:"
      ...
      : null;
    if (projectRef !== project.ref) return { schema: "api", rpc: "healthcheck", ready: false, status: "target_mismatch" };
    ...
    if (!response.ok) return { schema: "api", rpc: "healthcheck", ready: false, status: response.status };
    const payload = await response.json();
    const row = Array.isArray(payload) ? payload[0] : payload;
    return { schema: "api", rpc: "healthcheck", ready: row?.healthy === true, status: response.status };
  } catch {
...
const blockers = [
  ...
  ...(!healthcheck.ready && healthcheck.status === "target_mismatch" ? ["Configured Supabase URL does not match the approved development project ref."] : []),
...
console.log(JSON.stringify({
  ...
  blockers,
  nextActions: [
    ...(healthcheck.status === "target_mismatch" ? [`Set NEXT_PUBLIC_SUPABASE_URL only to the approved ${project.ref}.supabase.co target.`] : []),
