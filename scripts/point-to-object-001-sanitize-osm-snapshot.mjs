...
const SANITIZER_VERSION = "geoai-p2o-osm-snapshot-minimizer/1.0.1";
...
const NORMALIZATION_CONTROL_TAGS = new Set([
  ...
  "natural", "public_transport", "railway", "shop", "tourism", "type", "water", "waterway"
]);
...
function runParityNegativeControl() {
  const source = { type: "node", id: 1, lat: 1, lon: 1, tags: { public_transport: "platform" } };
  const projected = projectElement(source).projected;
  const controlHash = sha256(canonicalize(normalizationInput(source)));
  if (controlHash !== sha256(canonicalize(normalizationInput(projected)))) {
    throw new Error("public_transport parity positive control failed");
  }
  const mutated = structuredClone(projected);
  delete mutated.tags.public_transport;
  if (controlHash === sha256(canonicalize(normalizationInput(mutated)))) {
    throw new Error("public_transport parity negative mutation was not detected");
  }
  return true;
}

async function sanitize(directory) {
  const publicTransportParityMutationDetected = runParityNegativeControl();
  ...
  if (source.minimization) {
    throw new Error("Refusing to sanitize an already minimized snapshot");
  }
  const droppedKeys = {};
  const elements = [];
  ...
  const receipt = {
    ...
    acquisitionSource: {
      ...
      retainedAtReceiptWrite: true,
      requiredNextAction: "Verify minimized output, atomically promote it through a separately recorded finalization step, then delete the unminimized input."
    },
    minimizedSnapshot: {
      path: "raw-overpass-response.minimized.json",
    ...
    normalizationInputParity: {
      ...
      , publicTransportParityMutationDetected
    },
    ...
    configAtMinimization: { path: "case-config.json", bytes: configBytes.byteLength, sha256: sha256(configBytes) },
    minimizationTool: {
    ...
    acquisitionEvidenceLimitations: [
      ...
      "request endpoint, method and User-Agent are operator-recorded rather than contemporaneous machine-bound request evidence",
      "this sanitizer writes a distinct minimized output and does not itself prove final-path promotion or source deletion"
    ],
