...
const canApplyMigration = Boolean(process.env.SUPABASE_DB_URL?.trim()) &&
  ...
  ["pilot", "preview"].includes(process.env.GEOAI_ALLOW_SUPABASE_TARGET?.trim().toLowerCase() ?? "");
...
const output = {
  ...
  blockers: [
    ...
    ...(!canApplyMigration && !schemaApplied ? ["Migration apply is disabled until SUPABASE_DB_URL, GEOAI_ALLOW_SUPABASE_MIGRATION_APPLY=true and GEOAI_ALLOW_SUPABASE_TARGET=preview or pilot are set."] : [])
  ],
  nextActions: schemaApplied
    ...
    : [
        ...
        "Use npm run supabase:migrate:apply only after setting SUPABASE_DB_URL, GEOAI_ALLOW_SUPABASE_MIGRATION_APPLY=true and GEOAI_ALLOW_SUPABASE_TARGET=preview or pilot.",
