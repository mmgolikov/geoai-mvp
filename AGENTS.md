...
Supabase: `geoai-dev`, ref `pphdqkurxneyagvnnjdt`, eu-west-1. Production runtime reports Supabase env as not configured and uses local/API fallback. Fresh read-only evidence at 2026-07-16 11:31 UTC found the development project `ACTIVE_HEALTHY` on PG `17.6.1.141`, with 20 public tables/19 RLS, ten applied migrations/zero of three candidate migrations, zero Auth users, four buckets/zero `storage.objects` policies, 22 public-table `TRUNCATE` grants for each of `anon` and `authenticated`, 14 security advisors and 71 performance advisors. Development Data API containment and all candidate applies remain open; follow the draft operator runbook and do not infer Production/Auth/RLS/source readiness from this project. SOURCE-01 stages five RLS-closed custody tables plus a bounded caller-scoped source-release RPC; backfilled catalog entries default to `restricted`/`registered_unverified`, and only an explicit `approved` metadata projection can be returned—never arbitrary quality/lineage JSON, paths, URI, secrets or `client_viewer` access. AUTH-01B stages a cookie-only request read facade around exact project-key `api.current_project_access()`, the shared role/action kernel and approved-source release reads. All readiness flags remain false; no route, base-table, service-role or public-cache path is active. SOURCE-02 is a pure provider-neutral planner/receipt foundation with an empty registry, no fetch/env/secret/persistence and an unconditional Production denial. All provider writes remain blocked pending rights, custody/personas, trusted-worker, owner, exact-SHA, distributed rate/circuit and credential-broker evidence plus distribution/geometry/imagery gates when applicable. Review-only Storage policies use narrow `geoai_private.has_storage_project_role()` rather than direct protected-table joins, keep object reads operation-aware/no-listing and exclude `client_viewer`; they are not applied. The 71-assertion pgTAP plan adds 14 red-team checks for the exact source-release projection with ordered multi-event latest-status selection, explicit `revoked` and default-`sealed` projections, lower/upper pagination clamps, inactive organization/project denial, exact-project creator membership and artifact/status-event/ingestion-receipt update/delete immutability; its policy sweep covers all source custody/audit tables for `public` or authenticated-applicable policies. Documentation/evidence head `f39dcee18c2601b88ae51e061627925037d1aa77` preserves functional content SHA `631d72e0ec1323554fae7274f4328f92e2445289`; Quality Gate run `29497314994`, app job `87617248273` and Supabase clean-replay job `87617248275` succeeded, with artifact `8374954241` and digest `sha256:e3a5bbfdfb3278e7ed69cb8ab4ba2c8b6137fbbc091064ba38f6b05a5362c73a`. Clean replay alone does not close upgrade replay, live apply/Data API containment, live JWT/Storage/source personas, advisor parity or DB-01.
Figma/design: no Production design change is authorized by the current release.
Audit Preview `dpl_Gh5btUKs8yzySxvpxGnwXjMkYayK` is READY on exact documentation/evidence head `f39dcee18c2601b88ae51e061627925037d1aa77`; HTTP/runtime checks are evidence for their explicit routes only. Browser/mobile/keyboard/print evidence remains unclaimed because local headless launch is blocked by a system Unix-socket restriction; never relabel HTTP smoke as browser evidence.
